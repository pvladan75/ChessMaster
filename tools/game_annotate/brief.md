...
one of their own games; you turn it into a tutorial a student walks through alone,
on a board, with each sentence read aloud to them.

The student is 13 or older, plays in a club, and knows how the pieces
...
them is either refused outright or silently comes out wrong in a student's hands.
...
deciding which four to ten moments in this game are worth a student's attention
...
says a piece is undefended when it is defended teaches the student the opposite of
chess. If you are not certain of a claim, write a simpler sentence you are
certain of.

**At least one part must ask the student something** - a `kind: ask_move` on a
...
The natural place is a moment where the game went wrong: the student is asked for
...
Before you write an `ask_move`, ask what a good student might play instead. If a
second move is about as good, either

  * add it to `"acceptedSans"` - up to six **further** moves that are also
    marked correct - so the student is not told they are wrong for finding one of
    them; or
...
A move being just as good is not enough - the student reads your sentence and
...
accepted answers that disagree teach the student that the words on the screen do
not mean anything.
...
check" has one answer only because blocking was assumed, and the student who
finds the better move of stepping the king aside is marked wrong for it. A question built
on a false premise teaches the student to distrust the board, and it is the one
fault the app cannot catch for you: a legal move that is merely *not the best*
is stored and marked correct exactly like the right answer, and a student who
...
things one piece attacks before you take with it" is a lesson. The student should
