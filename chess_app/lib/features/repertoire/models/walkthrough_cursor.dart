...
class WalkthroughCursor implements MoveCursor {
  ...
  @override
  void last() {
    // „Never jump into a sibling variation" is `tourLastIndex`'s rule, and it
    // falls out of the walk rather than being special-cased here: a returning
    // beat stands on the fork, which is an ancestor, so the scan stops at it.
    onSelect(tourLastIndex(_paths, _tourBeats, index));
  }
  ...
  List<int> get _forwardIndices => tourForwardIndices(_paths, _stopIndex);

  /// The tour as `tour_walk.dart` sees it: paths, and nothing else.
  ///
  /// Rebuilt per call, like everything else on a cursor — it is constructed
  /// fresh in `build()` and holds no state, so there is nothing here to keep in
  /// sync with anything.
  List<List<String>> get _paths => [for (final stop in stops) stop.path];

  List<TourBeat> get _tourBeats => [
        for (final beat in beats)
          TourBeat(stopIndex: beat.stopIndex, returning: beat.returning),
      ];
  ...
  void takeBranch(int branchIndex) {
    ...
    final at = tourBeatOfStop(_tourBeats, found[branchIndex]);
    if (at >= 0) onSelect(at);
  }
