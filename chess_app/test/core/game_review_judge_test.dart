...
class _Engine {
  ...
  Future<List<AnalysisLine>> call(
    ...
    asked.add('p$i d$depth pv$multiPV'
    ...
    return [
      ...
      if (multiPV >= 2 && others.any((m) => m != bestUci))
        line(2, (second[i] ?? value[i] ?? at('0.00'))(reported),
...
SyzygyResult _tb(String fen, SyzygyCategory category,
...
Future<GameReviewResult> _review(_Engine engine,
    {BookLookup? book,
    TablebaseLookup? tablebase,
    int budget = kDeepeningSearches}) async {
  final result =
      await _judge(engine, book: book, tablebase: tablebase, budget: budget)
          .review(
...
void main() {
  ...
  group('what a mistake is', () {
    ...
    test('a move that mates ends the game: judged by the rules, not searched',
        () async {
      final ending = _Game(_rookEnding, _rookMoves);
      final engine = _Engine(ending)..walk(['M2', 'M2', 'M1']);
      final result =
          await _review(engine, tablebase: _Tablebase(const {}).call);
  ...
  group('the book', () {
    test('a book move losing 16 is theory, looked at deeper, and not marked',
        () async {
    ...
    test('a book move losing 22 at the walk but 18 deeper is not marked',
        () async {
      final engine = _Engine(italian)..chances([60, 60, 60, 38, 38, 38, 38]);
  ...
  group('the second look', () {
    ...
    test('a loss of 7 is a candidate, and 13 then 12 deeper is a mistake',
        () async {
    ...
    test('the budget goes to the move closest to its threshold first',
        () async {
    ...
    test('the played move heading the second line needs no search of its own',
        () async {
      final engine = _Engine(italian)..chances([60, 60, 60, 48, 48, 48, 48]);
    ...
    test('an engine that ignores searchmoves is read from the next position',
        () async {
      final engine = _Engine(italian)
  ...
  group('what could not be judged', () {
    test('a position that never answers leaves two moves unjudged', () async {
      ...
      expect(engine.wasAsked('p3 d16 pv1'), isTrue,
  ...
  group('seven men or fewer', () {
    ...
    test('a slower win is no mistake, whatever the engine says', () async {
      ...
      expect(engine.wasAsked('p0 d20 pv2'), isFalse,
          reason: 'the tablebase judges it; the engine is not asked again');
    });

    test('a win given away for a draw is, however small the engine\'s number',
        () async {
    ...
    test('a tablebase that does not answer leaves the engine, and is counted',
        () async {
      final engine = _Engine(ending)..walk(['M1', '+6.00', 'M1']);
      final tablebase = _Tablebase(const {});
      final result = await _review(engine, tablebase: tablebase.call);
  ...
  group('the store', () {
    test('a review run again asks the engine nothing it already answered',
        () async {
      final engine = _Engine(italian)..chances([60, 60, 60, 53, 53, 53, 53]);
      engine.alone[2] = (d) => d >= 24 ? c(48) : c(47);
      ...
      Future<GameReviewResult?> run(EngineAnswerTally tally) => GameReviewJudge(
            analyzer: store.wrapMoves(engine.call, engine: name, tally: tally),
            book: _noBook,
  ...
  group('what the review leaves on the game', () {
    ...
    GameReviewResult result() => GameReviewResult(reviewDepth: _d, moves: [
          move(0),
          move(1, lost: 30, reason: MistakeReason.lostChances, unsettled: true),
          move(2, lost: 12, reason: MistakeReason.lostChances, replyPv: 'f8c5'),
          move(3, lost: 25, reason: MistakeReason.lostChances, replyPv: 'd2d4'),
          move(4, judged: false),
          move(5, lost: 4),
        ]);

    test('?? on exactly the review\'s mistakes, the better line beside them',
        () {
      final marked = GameAnalysisWalkerService()
          .markMistakes(chain: chain, result: result());

      expect(marked, 2);
      expect(
          [for (final n in chain) n.nag], [null, null, '??', '??', null, null]);
    ...
    test('only the side chosen', () {
      ...
      expect(marked, 1);
      expect(
          [for (final n in chain) n.nag], [null, null, '??', null, null, null]);
      expect(chain[1].children, hasLength(1));
    });

    test('today\'s puzzles from the mistakes, worst first', () {
      final puzzles = LocalPuzzleExtractorService()
          .buildPuzzlesFromReview(result(), maxPuzzles: 5);

      expect([for (final p in puzzles) p.sourcePlyIndex], [3, 2]);
      expect(puzzles.first.fen, chain[3].fen);
      expect(puzzles.first.refutationSan, 'd4');
      expect(
          LocalPuzzleExtractorService()
              .buildPuzzlesFromReview(result(), maxPuzzles: 1)
              .map((p) => p.sourcePlyIndex),
          [3]);
    });
  });
}
