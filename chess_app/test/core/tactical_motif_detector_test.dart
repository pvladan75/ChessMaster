...
void main() {
  group('TacticalMotifDetector Full Geometric Detection Unit Tests', () {
    ...
    test('6. Detects hanging Queen via SEE even when defender count matches attacker count', () {
    ...
    test('8. Flags a piece the mover left hanging for the opponent (favorsMover=false)', () {
      ...
      final blunder = result.findings.firstWhere(
        ...
        orElse: () => throw StateError('expected a favorsMover=false hanging finding'),
      );
    ...
    test('9. explainMove flags a freshly hung Queen as "created"', () {
      ...
      final diff = detector.explainMove(beforeFen: beforeFen, afterFen: afterFen, lastMoveUci: 'd1d5');

      final blunder = diff.created.firstWhere(
        (f) => !f.favorsMover && f.motifs.contains(TacticalMotif.hangingPiece),
        orElse: () => throw StateError('expected a newly-created hanging finding'),
      );
      expect(blunder.affectedSquares, contains('d5'));
    });
    ...
    test('10. explainMove flags a hanging piece the move fixed as "resolved"', () {
      ...
      final diff = detector.explainMove(beforeFen: beforeFen, afterFen: afterFen, lastMoveUci: 'e5d3');
    ...
    test('11. Detects deflection: attacking the sole defender exposes what it guards', () {
    ...
    test('12. A capture that walks into immediate mate is not a real threat', () {
      ...
      final rookFlaggedHanging = result.findings.any(
        (f) => f.motifs.contains(TacticalMotif.hangingPiece) && f.affectedSquares.contains('f5'),
      );
      expect(rookFlaggedHanging, isFalse);
    });

    test('13. describeMoveDiff drops a low-significance finding when something bigger happened', () {
    ...
    test('14. describeMoveDiff keeps a low-significance finding when it is all that happened', () {
      const diff = MoveMotifDiff(
        created: [
          MotifFinding(
            motifs: [TacticalMotif.hangingPiece],
