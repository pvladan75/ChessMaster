...
void main() {
  ...
  group('what the card must not become', () {
    test('the platform question has exactly one home', () {
      // The predicate has one home so that „which screens make sense on a
      // phone" stays a one-line change. A second copy is how that stops being
      // true, quietly.
      ...
      for (final file in Directory('lib')
        ...
        // `engine_settings_dialog.dart` asks the same question about a
        // different thing and predates all of this.
        if (file.path.replaceAll(r'\', '/').endsWith(
      ...
      expect(offenders, isEmpty,
          reason: 'read isTutorialStudioAvailable; do not write the platform '
              'test a second time');
    });
    ...
    test('the card owns its own strings and its own dialogs', () {
      ...
      final tab = File('lib/widgets/home/biblioteka_tab.dart').readAsStringSync();
