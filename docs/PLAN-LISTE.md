...
| 3 | **Pattern A onto the card lists**: `LibraryList`'s rows, `HomeworkListScreen`, `SavedPuzzleSetsDialog`. The row content moves off `ListTile` onto a card that has somewhere to put a second fact; the actions stay where `actionsBesideFrom` already puts them | `[implementer]` | `library_list_test.dart` green **unchanged wherever it asserts behaviour**, amended only where a finder names `ListTile` — and the brief says which, so a silently rewritten assertion is visible in the diff. New: the room's column still draws one column at its real width, pumped **from the room** and not from a bare `LibraryList`; the library at 1400 draws three rows on one line, proven by y-offset equality of three known titles. The existing 360 dp overflow test kept and extended to 1400 |
