...
Practise    the hub, unchanged — except the progress line and „Retry failed"
            on its cards, which are `PLAN-NAPREDAK-VEZBI.md`
...
| 1 | S2 — *Use in a tutorial* in Analysis; the four buttons and their three `_` methods become one menu over the same three flows; `analysis_studio_screen.dart` class doc corrected | implementer | `analysis_teach_menu_test`: one menu, its rows by platform, each row reaches the flow it names (fake the client, assert the request — rule 7); the four old tooltips absent from `lib/` |
| 2 | S1 (Windows half) + S4 + S5 + names — the room loses *Create tutorial* and *Edit positions*; `CreateCourseDialog` deleted; dead dialogs deleted; Preparation's one door; the strings table applied | implementer | `dead_ui_test`: the deleted strings absent; `home_map_test` rows for A; the vocabulary anchors green |
| 3 | S3 — `LibraryScreen` and the shared library list used by the room's column | implementer | `library_lists_everything_test`: one saved position from the room, one from a scan, one analysis, one recording, one tutorial — all five appear, filter by kind, and the room's column shows the three that fit a board |
| 4 | S6 — the panel to Sessions; People trimmed; manual pages 1, 2, 5, 6, 7, 8 rewritten (this is **Variant A complete**) | implementer (code), lead (manual) | `home_map_test` A rows; `manual_labels_test`; a new `manual_places_test` that reads each page's *Where* line against the map |
| 5 | Variant B — the shell: `Home`, `Teach`, `Analyse`, five destinations, shortcuts, back history, landscape at four phone sizes; manual pages 0 and 1 | lead (shell), implementer (tabs' bodies) | `home_map_test` B rows; `home_landscape_test` re-measured; every deep link in `app_routes.dart` still lands |
