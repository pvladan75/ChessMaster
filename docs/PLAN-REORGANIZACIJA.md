...
| 3 | S3 — `LibraryScreen` and the shared library list used by the room's column | implementer | **3a done 17.9.2026**, merged `2671241`: the server's view gains `tutorial` and `recording` (backend gate 6 tests, 1406 → 1412); `LibraryList` (gate 9 tests) and `LibraryScreen` at `/library`, the door on the Library tab, the tutorial row's five actions lifted into `TutorialRowActions`; a saved analysis opens whole through the Analysis screen's new `initialTree` (`8c3d418`). **3b — the room's column onto the shared list — is open**, after phase 5. Master after 3a: 2859, 1 skipped, analyze 26 |
