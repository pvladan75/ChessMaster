...
| 6a | §7.1 — `TutorialDraftController` extracted | lead | **done 17.9.2026 (lead, Fable).** `services/tutorial_draft_controller.dart`, a `ChangeNotifier` holding the draft, the open part and its cursor, the history, the step ids a save handed out, the saved version and the last move; every mutation is a method, the fields write through (no `_syncSelectedSection`), two signals (`notifyListeners` redraws, `generation` says „rebuild your fields"). The desktop screen is a layout over it: a board controller, the drawing mode, the text fields and their epoch, the dialogs and the messages — 2693 → 2219 lines. The 543 studio tests passed **unedited**; `test/tutorial_draft_controller_test.dart` builds a two-part tutorial with a question, undoes past a save, splits a part, drops a line, removes an answer — 14 tests, no frame, two proved by mutation |
| 6b | §7.2 — the phone layout | implementer | **done 17.9.2026**, merged `c361694` (worker in a worktree, graded by the lead). `screens/tutorial_studio_phone_layout.dart`, a `part` of the screen: Line \| Task \| Parts over the same controller, the board and a dense move strip on top, the tabs pinned, the open tab under them; landscape through `LandscapeBoardLayout`; the drawing bar inside the Line tab (under the board it filled the screen); the overflow holds Undo, Redo, Discard changes, Preview, Record narration, Export video, Save as .pgn, Position setup; chosen for a touch platform narrower than the wide breakpoint, a Windows window of that width keeps the desktop's narrow branch. The gate `test/tutorial_phone_layout_test.dart` (4 tests) is green and its Save mutation red; the worker found two defects in the gate — the override reset in a `tearDown` runs too late, and the phone half's draft leaked into the desktop half through the device slot — stopped, and both were the gate's. Not built: the Tree as a read-only dialog (§7 mentions it; no gate asks for it) |
| 6c | §7.3 — retirement of the two editors and the platform guard; the Android door in the room and the Tutorials card | implementer | **done 17.9.2026**, merged `8af285f` (worker in a worktree, graded by the lead). `LessonStepEditorPanel` (824 lines) and `tutorial_studio_availability.dart` deleted; `openTutorialEditor` pushes the studio everywhere; the Tutorials card draws itself on every platform; the teach menu lost `studioAvailable`; the manual's three Windows-only sentences rewritten. The gate `docs/gates/one_editor_test.dart` (grown to three tests: the files, no identifier in `lib/` code, one route in the door) is in `test/` green. The panel's four test files went with it (30 tests); one rule they held — a question from a list may keep its line — re-homed on `TutorialDraftController.validate()`; the landscape test lost its six panel cases, three door and entry tests went. Backend unchanged |
| 7 | Live pass, one `TODO-provera` item per phase, on Windows and on a phone | owner | **the only open row**: items 175, 176, 177, 178, 179 in `docs/TODO-provera.md` |
