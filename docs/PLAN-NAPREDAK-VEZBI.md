...
Proposed 17.9.2026 at the owner's request, as a lean design. Nothing is built.
...
| 1 | Backend writes: `/submit` inserts the attempt row with `source = puzzles.type`; `/attempt` accepts `source`, `skipped`, `hinted`; two by-id routes (`puzzles`, `endgame_puzzles`); `GET /api/puzzles/progress`, `GET /api/puzzles/retry` | implementer | route tests through `supertest`-style calls against the seeded pool: one call per drill, then `progress` reports it under the right `source`; `.env` moved aside (CI's environment) |
| 2 | App writes and reads: the five drills post their outcome and skips through one `PuzzleAttemptApi` (fake the client, assert the request — rule 7); hub cards show the line and the button; retry mode in the drill screens (`retryIds` on the route as a query, not a new path) | implementer | widget tests: a card with no data draws no line; a card with `toRetry: 9` draws the button; retry mode serves the ids in order and a solve removes one; the request body per drill |
