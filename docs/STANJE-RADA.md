...
Vlasnik je podsetio na dogovor stariji od nivoa naloga i kvota: pratiti kako on
sam troši spoljne servise, da bi model naplate imao broj od koga da krene
(`CENA-I-PRETPLATA.md`, §5 — „prvo mesec dana merenja"). Merač je postojao od
avgusta (`usage_counters`, `getUsageReport`, `tools/status/status.js`), ali
niko ga nije gledao, a tri stvari koje koštaju ili se broje kod tuđeg servisa
nije merio niko. Isti dan vlasnik je rekao dve stvari koje ovaj odeljak
prihvata: **Gemini je relikt** (ne koristi se; dvoja vrata još stoje —
„Generate AI comment" u Analizi i „Ask" u repertoaru — i idu u sledeću turu
brisanja, ne ovde), i **aplikacija je u test fazi, pa ekran za budućeg
korisnika može odmah.**

**Server** (grana `api-metering`, urađeno u worktree-u jer je nodemon radio):

- **Azure znakovi.** `speakBeats` javlja `onSynthesised({ provider, characters,
  sentences })` — samo rečenice koje nisu bile u kešu i sada imaju fajl, iz
  `finally`, pa i prekinut film prijavi ono što je platio; povratni poziv koji
  baci grešku ne obara film. Ruta izvoza knjiži `tts_<provider>_characters`
  treneru **u trenutku izgovora**, ne posle uspešnog crtanja: Azure je naplatio,
  a klipovi ostaju u kešu za sledeći pokušaj. Uzorak glasa (`/lessons/tts/sample`)
  knjiži isto; keš pogodak ne knjiži ništa. Cena u `.env.example`
  (`tts_azure_characters`, zasad cenovnik od $15 po milionu).
- **`provider_requests`** (provajder, dan, broj) — nova tabela, `initDB` je
  pravi: `lichess_tablebase`, `local_tablebase`, `lichess_cloud_eval`,
  `lichess_games` (jedan tok po uvozu), `lichess_user`. Po danu, a ne po nalogu,
  jer domaći suđen na serveru, pregled sa telefona i dril u sobi stižu do istog
  servisa, a samo neki od tih poziva znaju čiji su. Servisi dobijaju `onRequest`
  (podrazumevano niko — test ne dira bazu), `server.js` ih jednom priključi
  (`wireProviderMeters`), `routes/userGames.js` uvoz i pogled na protivnika.
  Keš pogodak se ne broji; brojač koji baci grešku upiše se u log i ništa više.
- **Status alat**: Azure znakovi naspram F0 (500.000), odeljak
  „Free services this month" iz `provider_requests` (mesec i danas; bez tabele —
  rečenica „restart the server once"), i „Previous months" — pet prethodnih
  meseci po metrici.

**Aplikacija**: `UsageScreen` na `/usage`, vrata Settings → `ACCOUNT` →
`Usage this month` (samo prijavljen nalog). Dva zahteva sa tokenom —
`/billing/entitlements` i `/billing/usage/me` — i tri kartice: `Plan` (nivo,
od kog datuma se broji), `Monthly limits` („iskorišćeno / limit", „No limit",
traka punjenja po svetlini, ne po boji), `Also counted` (glas u minutima,
videi, minuti rendera, naracija u znakovima — svi provajderi u jednom redu,
strane, tokeni). Metrika za koju ekran nema reč prikazuje se očovečena, nikad
se ne ispušta; metrika sa kvotom nikad se ne pojavljuje dvaput. Cena se ne
prikazuje — server je i ne šalje. Bez servera: „Could not load your usage." i
`Try again`. Reč **Usage** je u rečniku; priručnik pominje red.

**Brojevi**: backend **1747 → 1772** bez baze (20 u
`api_usage_metering.test.js`, slučaj 5b izvoza, dva slučaja uzorka glasa —
knjiženje, i to da merač koji ne zna provajdera ne sme da obori uzorak — tok
uvoza, i jedan `sources_compile` za nov fajl `services/providerUsage.js`; sa
bazom 1930 izvedeno, ne mereno); aplikacija **4129 → 4139**
(`usage_screen_test`, 10; pun prolaz u worktree-u). Mutacije: 12 na serveru,
sve uhvaćene; 6 u aplikaciji, jedna preživela i popravljena — vidi
`LESSONS.md`, 26.9.2026.

**Šta se i dalje ne meri, rečeno otvoreno** (tabela u `CENA-I-PRETPLATA.md`,
§1): pozivi koje aplikacija šalje direktno (cloud-eval i `stockfish.online` u
onlajn režimu motora, uvoz sa chess.com/Lichessa, tablebase kad server nije
dostižan) — server ih ne vidi; mejlovi; prostor na disku. DeepSeek se knjiži
kao ukupni tokeni, a naplaćuje ulaz i izlaz različito — procena, ne račun.

**Sledeće**: vlasnikova provera uživo [248.1]–[248.4]; posle spajanja **jedan
restart servera** (tabela); a kroz mesec dana `node tools/status/status.js` je
broj od koga cena kreće.

---

## Redosled varijanti i grana tutorijala — 26.9.2026, plan; faza 0 u kodu

Vlasnik je u mapi svog tutorijala („proba 2") video lanac 2 → 3 → 4 i nije
znao u kom smeru idu isprekidane linije; zatim je predložio da se redosled
sporednih grana menja — ne samo „glavna/sporedna" — i to u celoj aplikaciji.

**Faza 0, u kodu i spojena (4103, analyze 22):** deo koji se vraća visi o
potezu koji njegov red imenuje, a svaka linija se završava strelicom na delu u
koji vodi. Obe varijante su nacrtane i poslate pre spajanja. Provera uživo:
[247.9].

**Plan: [PLAN-REDOSLED-GRANA.md](PLAN-REDOSLED-GRANA.md).** „Move variation
earlier / later" u meniju stabla poteza — desni klik na Windowsu (meni kod
pokazivača), dugi pritisak na Androidu (list odozdo), u oba prikaza stabla
(grafik i PGN); u tutorijalu ista komanda pomera celu granu (deo i sve što
visi o njemu) — u kartici Tree, i na telefonu dugim pritiskom na red mape.
Strelice ↑ ↓ u zaglavlju dela ostaju kakve su (vlasnik). Ne u repertoaru (red
se tamo ne čuva). Ideja „ceo tutorijal kao jedan PGN" nije u planu (§7 — spoj
gubi rečenice). **D1–D7 odgovoreni po preporuci 26.9.2026** (D7: kartica PGN ostaje jedan deo kao tekst).

**Faze 1–3 u kodu 26.9.2026** (grana `redosled-grana-faza-1`; aplikacija
**4129**, analyze 22): „Move variation earlier / later" u jednom meniju oba
prikaza svakog stabla (Analiza, kartica Tree tutorijala; ne repertoar); kartica
Tree tutorijala crta celu porodicu delova i komanda pomera deo sa svim što visi
o njemu; na telefonu dugi pritisak na red mape. Priručnik o Analizi je
imenovao „Set as main line" (natpis ekrana partije) — ispravljeno. Provera
uživo: [247.10]–[247.12].

## Delovi tutorijala kao mapa, i deo kao jedna linija — 25–26.9.2026, faze 0–5 u kodu, ostaje provera uživo ([247.1]–[247.8])
...
fixture-i sa svojim testom (→ 4024).

**Faze 1–5 u kodu, 25–26.9.2026, spojene u `master` lokalno (nije push-ovano).**
Aplikacija **4101**, 1 preskočen; backend **1747 / 1905**; `analyze` 22. Svaka
faza ima svoj odeljak u planu, a brojeve i lekcije u `LESSONS.md`.

- **Faza 1** — drugi potez u delu pravi nov deo odmah posle njega. Na vlasnikovu
  reč: dok tab „PGN" ima neprimenjen tekst, **ni jedan izlazak iz dela** nije
  dozvoljen (potez, drugi red, novi/kloniran/obrisan deo, delovi iz drugog
  tutorijala, „Insert a line here", „Position setup", Undo/Redo) — kaže se
  „Apply or discard the text in the PGN tab first."; pored „Apply" je
  „Discard"; Ctrl+Z u tom polju vraća kucanje umesto izmene tutorijala.
- **Faza 2** — `splitAtForks` iza svih drugih vrata: „Apply" u PGN tabu, obe
  predaje iz Analize, uvoz JSON i PGN fajla, i „Add this line to a tutorial…",
  čija ruta `POST /lessons/:id/steps` sada prima `steps` — sve ili ništa.
- **Faza 3** — mapa delova (trake, puna/isprekidana linija, kvadrat/krug,
  „you are here"), dugme „Part N starts here" u Flow i zaglavlje dela sa vezom.
  Na telefonu su akcije dela jedan red iznad mape.
- **Faza 4** — mapa u svojoj koloni levo od table kad tabla zadržava veličinu
  (od 1504 px na visini vlasnikovog prozora; `AppSpacing.md` je 12, ne 16 kako
  je plan računao); naslov u traci, oznake i jezik iza „Details…" (isti prozor
  kao na telefonu); zaglavlje otvorenog dela; „Part 3 of 8" ispod table.
- **Faza 5** — reči: priručnik `site/mislisha/manual/write-a-tutorial.html`,
  `UPUTSTVO-STUDIO.md`, i osam stavki `[247.1]`–`[247.8]` u `TODO-provera.md`
  (Teach → Tutorial studio).

**Za vlasnika, otvoreno:** stavka [246.11] kaže „Svaki potez odigran na tabli
ide u liniju dela" — to više ne važi za potez odigran tamo gde linija već ide
dalje. Stavka nije menjana (alat prepoznaje stavke po tekstu); [247.1] to kaže.
Na 1366 × 768 mapa ostaje u desnom oknu i pokazuje oko dva i po reda.
**Sledi faza 6: vlasnikova provera uživo.**
...
tutorijal nego izrenderovan video (samostalno ili kao stavka domaćeg); delovi
su samo `show`, `ask_move` i `ask_choice` se izbacuju; beleži se da li je
učenik skinuo video (ne i da li ga je pogledao); ko hoće da stavi učenika pred
zadatak, pravi zadatak. Posle toga, dogovor o crtanju delova kao grafičkog
stabla poteza. P.S.: pregledao je tutorijalske stavke u `TODO-provera.md` i
potvrđuje da su onakve kako je dogovoreno — ali iskustvo treba da bude
jednostavnije.

**Plan: [PLAN-TUTORIJAL-VIDEO.md](PLAN-TUTORIJAL-VIDEO.md).** Ništa u kodu.
§4 ima 14 odluka, svaka sa preporukom; **vlasnik je istog dana odgovorio „sve
po preporuci, kreni sa fazom 0"**.
Tri koje najviše menjaju: **D2** — film tutorijala više ne briše tajmer od 14
dana (inače poslat video nestane pre nego što ga učenik skine); **D5** — slanje
traži video, a izvoz videa je plaćen, pa trener na besplatnom nivou ne može da
pošalje ništa od tutorijala; **D6** — trenerovi tutorijali nestaju iz
Biblioteke njegovih učenika.

**Vlasnikova druga reč istog dana:** aplikacija je u test fazi, sav materijal
u bazi sme da se obriše ako smeta, ništa postojeće ne mora da se prilagođava.
Zato se ništa ne pretvara: sačuvani delovi-pitanja se brišu (D7), fajl sa
pitanjem se pri uvozu odbija (D11), a **faza 1 je povučena**.

**Faza 0 urađena 25.9.2026** (samo čitanje, jedna `READ ONLY` transakcija): 4
tutorijala; 2 dela-pitanja (`ask_move`), oba u jednom tutorijalu; nijedan
`show` deo nema zadatak ni rešenje; jedan poslat tutorijal (istog dana, bez
ijednog pokušaja); `review_items` prazan; dva filma na disku, i 12 starih
koje nijedan red ne imenuje. Usput: `db.js:961` pri svakom startu upisuje
`step_key` i stavkama „Play it out" (16 redova) — bezopasno, odlazi sa
kolonom u fazi 2. Osnova u worktree-u: server 1737 / 1888, analyze 23 info.

**Faze 2 i 3 u kodu 25.9.2026** (lead, u jednom worktree-u). Učeniku ide
video: poslati tutorijal je jedna stavka, urađena kad server pošalje poslednji
bajt fajla tom učeniku (`GET /assignments/:id/video`, link sa sopstvenom
namenom i imenom zadatka u tokenu); prekinuto skidanje, `HEAD` i deo fajla ne
računaju se. Tutorijal bez videa se ne šalje ni sa jednih vrata. Film
tutorijala više ne briše tajmer od 14 dana. Učenik ne vidi trenerove
tutorijale. Obrisani: đakov pregled tutorijala i „Preview tutorial", koraci
`/step/…`, „Due for review" sa `review_items` i `/reviews`, glas po jeziku
tutorijala na uređaju. „Turn this part" je u redu svakog dela (bilo je samo u
„Preview tutorial"). Brisanje tutorijala kaže koliko učenika još nije skinulo
video. Brojevi: aplikacija **4177 → 4076** (−115 / +14); server 1737 → 1735 bez baze, 1888 → 1891
sa bazom; analyze 22 info. Pouke su u `docs/LESSONS.md` (25.9.2026).
**Pre provere:** backend se pokreće sa novim kodom — `initDB` tada briše
`review_items` (0 redova) i kolone `step_key`/`revealed_at`.

**Faza 4 u kodu 25.9.2026** (lead, u glavnom checkout-u). Studio pravi samo
delove koji pokazuju: nema `Task type`, zadatka za učenika, ponuđenih
odgovora, „Find the move" i „Choose the answer"; potez na tabli je uvek potez
linije; na telefonu su kartice `Line | Parts`. Server odbija deo koji pita
rečenicom „A part only shows a position and a line; a question is an
exercise." — na čuvanju, izmeni, dodavanju dela i kopiji — i više ne čuva
zadatak, rešenje ni odgovore. JSON uvoz sa pitanjem odbija se uz broj dela;
PGN uvoz više ne nudi pitanja od grešaka; tutorijal iz partije više ne pita
„za učenike ili za video". „Add to tutorial" u Biblioteci pretvara zadatak
pozicije u prvu rečenicu dela, pa ga video izgovara. **Podaci:** dva dela
`ask_move` u tutorijalu 77 obrisana (10 → 8 delova), jednom transakcijom
posle ponovnog brojanja. Brojevi: aplikacija **4076 → 3981**; server 1735 →
1730 bez baze, 1891 → 1886 sa bazom; analyze 22 info. Provera uživo:
[246.11]–[246.15]. Sledeća je faza 5 (tutorijal iz partije ne pravi pitanja
ni u kosturu). Ništa nije commit-ovano.

Izmereno uz plan: pitanje je u filmu već samo tabla sa zadatkom ispod
(`_captionOf`); „Due for review" na Home-u hrani isključivo đakovo prolaženje
kroz tutorijal, pa odlazi s njim; „Preview tutorial" je jedino mesto gde se
okreće jedan deo, pa to prelazi u red dela. Vlasnikovih 29 „u redu" od
25.9.2026 u alatu (`Home — Domaći i lekcije`) su uglavnom đakov pregled
tutorijala koji ovaj plan briše — ne proveravaju se ponovo.

## Spisak za proveru uživo preuređen — 25.9.2026

Vlasnik: „uradi trijažu starih provera, sredi redosled, napiši u stavkama
putanju gde i šta treba da uradim — često ne znam na šta se odnosi".

**Šta je nađeno.** `TODO-provera.md` je imao 2008 stavki u 251 odeljku, od
toga 1825 otvorenih. Vlasnik je u alatu za proveru (`mislisha-test\qa`, van
repozitorijuma) njih 789 već bio označio „u redu" — nijedan od tih odgovora
nije bio prenet u dokument, pa je spisak izgledao tri puta duži nego što jeste.

**Šta je urađeno.** Svaka otvorena stavka je dobila presudu, uz dokaz iz koda,
commit-a ili kasnijeg dokumenta:

- **765 štiklirano** — 750 „u redu" bez komentara i 15 čiji komentar potvrđuje
  proveru. Ostala 24 „u redu" sa komentarom („ne znam na šta se misli", nalaz)
  su prepisana, ne štiklirana.
- **210 prevaziđeno** — funkcija je promenjena ili uklonjena pre nego što je
  provereno: repertoar koji se gradi na tabli (15–16.9), snimanje iz sobe,
  puzzle sets, zadatak od više poteza, kucanje koda sobe, srpska množina,
  prekid napuštenog rendera, Syzygy provera i sejanje repertoara iz arhive.
- **850 prepisano u 847 stavki**, poređanih po mestu u aplikaciji (oblast, pa
  ekran), svaka u istom obliku: *O čemu se radi*, *Gde* (putanja od početka
  aplikacije, natpisi tačno kako stoje u aplikaciji — mašinski provereno da
  postoje u kodu), *Uradi*, *Treba da vidiš*, *Potrebno*. Stari broj stoji uz
  stavku, npr. [243.2].

Stari spisak je ceo, sa sudbinom svake stavke, u
[arhiva/TODO-provera-26.8-do-25.9.2026.md](arhiva/TODO-provera-26.8-do-25.9.2026.md).

**Alat za proveru.** `spoji.py` sada ispušta stavku koju dokument više ne nudi
**a na koju niko nije odgovorio** (ranije je ostajala kao „netestirano" pored
svog prepisa), a odgovorene stavlja u jedan odeljak, „Arhiva odgovora".
Zadržano 887 „u redu", 50 „problem" i 4 „preskočeno"; ispalo 16 praznih
„netestirano". Rezerve: `stavke-rezerva-20260925-1239.json` i
`spoji-rezerva-20260925-1239.py` u istom folderu. `pokreni.ps1` [3] pokazuje
novi spisak.

**Nalazi iz trijaže koji traže posao, ne proveru:**

1. `Save tutorial` nikad nije sivo, ni kad nema izmena — vlasnikova prijava od
   11.9.2026, još tačna ([151.6]).
2. Priručnik na sajtu (`site/mislisha/manual/`) se ne slaže sa aplikacijom:
   `analysis.html` kaže `Set as main line` (Analysis kaže `Promote to Main
   Line`), pripisuje „Blunder detection" dijalogu „Automatic Position
   Analysis" (pripada „Review entire game") i kaže da se motor gasi u Settings
   (od 17.9 u „Panels"); `practice.html` opisuje „Judge moves → Punish" u „My
   mistakes" (ne postoji na tom ekranu), „Include online games" pripisuje
   uvozu arhive (pripada izboru završnica) i kaže da „My games" pravi „Game
   mistakes"; `preparation.html` pominje dugme `Stop` za skeniranje koje ne
   postoji. `manual_labels_test` proverava samo da natpis postoji negde u
   `lib/`, ne i na ekranu koji priručnik imenuje.
3. Ekran prečica na tastaturi, grupa „Endgame trainer": piše „Restart" i
   „Tablebase lookup", dugmad se zovu `Start over` i `Tablebase findings`.

## Zagonetke iz partije, faza 1b: tutorijal na pravilu greške — 25.9.2026, u kodu, ostaje provera uživo (stavka 245)

Tutorijal iz partije više ne bira momente po pešacima (klizač „Teach a move
that cost X pawns" je izbačen, na vlasnikovu reč). Momenti su **iste greške
koje obeležava pregled partije** — isti sudija (`GameReviewJudge`), sa
produbljivanjem — i **jedini potezi koje je igrač našao**; najviše 8. Tačni
odgovori na pitanje su svi potezi koji sami nisu greška, pa se pitanje
postavlja ređe (70% momenata umesto 97%). Jedini potez se priča kao takav
(„partija je baš njega odigrala"). Podrazumevana dubina je 20. Deset test
partija ima nove odgovore modela (deset DeepSeek zahteva). Plan: faza 1b,
„Built".
...
Pozicije sa **5 figura i manje** backend sada pita naš `lila-tablebase` nad
tabelama `D:\syzygy\3-4-5`; 6 i 7 i dalje Lichess, kao pre. Ako naše tabele
ne odgovore, pita se Lichess i log to kaže. Aplikacija pita backend
(`GET /api/tablebase`), a Lichess direktno samo bez prijave ili bez mreže.
Rust je instaliran na ovom računaru; `lila-tablebase` je kloniran u
`D:\Projekti\lila-tablebase` i građen sa malom zakrpom za Windows
(`tools/lila-tablebase/`). Pokreće se iz `pokreni.ps1`, stavka 5.
...
**3a** server (`POST /review-words`, DeepSeek, sopstveno pravo
`ai_review_words` i brojači `ai_review_words` / `ai_review_tokens`, kvote kao
kod tutorijala — mesto za odluku o ceni); **3c** sat poteza (`[%clk]` i
`TimeControl` ostaju u partiji); **3b** aplikacija — kućica „Comment key
moments with AI" u „Review entire game" (vidi se uz Blunder Alert ili
zagonetke, ugašena po difoltu), najviše 10 momenata (greške po izgubljenim
šansama, pa jedini nađeni potezi), jedan zahtev po pregledu, svaka rečenica
kroz proveru tvrdnji (potez van linija tog momenta se odbija), reči na potezu,
na „Better move", i na novoj liniji „Refutation"; tuđ komentar se nikad ne
prepisuje; sačuvane zagonetke nose objašnjenje (`review.words`), koje faza 5
već crta posle odgovora. Kad server odbije, oznake i zagonetke su tu, a
dijalog kaže da komentara nema i zašto.

**Za proveru uživo treba restart servera** (nova ruta; šema ista). Detalji,
kapija i šta nije pokriveno: u planu, faza 3, „Built, 25.9.2026".

## Pravilo greške, navike u otvaranju i odgovori engine-a na disku — 24.9.2026, u kodu, ostaje provera uživo (stavke 237–239)

**Gde smo.** `PLAN-ZAGONETKE-IZ-PARTIJE.md`: faza 0 izmerena u celosti
(desktop, velemajstorske i klupske partije, vlasnikove, i telefon), vlasnik
izabrao `A` = 10 na dubini 20, `B` = 15, „u knjizi" = bar 10 majstorskih
partija, `A_gross` 20, promašen mat u ≤5 je greška; Fableov nezavisni pregled
je u `REVIEW-ZAGONETKE-FABLE.md` i upisan u plan. **Faza 1 je podeljena na
1.1 / 1.2 / 1.3; 1.1 (skladište odgovora engine-a, `EvalCache` na disku) je u
...
na telefonu se drži protiv Analysis-a čiji `detach` zaustavlja svaku pretragu,
prekinut pregled nastavlja iz skladišta); **budžet produbljivanja je u
pretragama, ne u sekundama**, isti na oba uređaja. Telefon je izmeren 5–5,5×
sporiji od desktopa (380 s za 115 pozicija na dubini 20).

**`PLAN-MOJE-PARTIJE.md` §9** (stablo sopstvenih partija): 9.1–9.4 u kodu —
presude navika (`opening_judgements`), sudija na desktopu, „izgubljene navike"
u dril; 9.5 (uklanjanje Lichess sudije) čeka vlasnikovo da, 9.6 je provera
uživo (stavka 237).

**Usput:** „Review entire game" je od 22.9 obećavao komentare koje više ne
piše — sada Start čeka Blunder Alert ili zagonetke i kraj kaže šta je nađeno
(stavka 238). Skladište (stavka 239) čuva dubinu koju je pretraga **dostigla**,
a ograda naloga stoji na poslednjem koraku upisa — i u tutorijalovom
`facts_store`.

## Knjiga čiji font nemamo čita se kao slike — 23.9.2026, u kodu, ostaje provera uživo (stavka 236)

**Povod:** vlasnik je probao druge knjige iz `D:\chess books` i pet ih je
odbijeno kao „font we cannot read yet". Font je bio poznat (`DiagramTTFritz`),
ali mapa napisana sa jedne knjige; popravljeno istog dana (commit `b0f3e55`,
`docs/LESSONS.md`). Vlasnik je onda odlučio: **knjiga čiji font nijedna mapa ne
čita ide putem slika**, samo tada. „Print to PDF" nije zamena — izmereno,
Microsoft Print to PDF zadržava tekst i font, samo ga preimenuje.

**Šta je urađeno (samo server):** kad slike u PDF-u ne daju nijednu tablu,
server sam nacrta strane (300 dpi, u posebnom procesu sa rokom) i traži table
na crtežu kao na skeniranoj strani; dalje ide postojeća kalibracija i čitanje.
Dve stvari su tek merenjem nađene: pdfjs nosi **sopstvenu kopiju** biblioteke
za crtanje, i mešanje dve kopije ruši ceo proces (nativno, bez poruke) — zato
poseban proces i jedna kopija; i test „da li je šahovnica" je tražio kontrast
25, a šrafura fonta daje 22–26 — sada 15, a na Silmanovom skenu to ne menja
nijednu stranu. Rezultat na 146 tabli sa poznatim odgovorom: sve nađene, jedno
pogrešno polje i ono označeno; kroz rutu na četiri knjige 63 pozicije, nula
grešaka. Usput: *Grandmaster Codex* (table linijama, §6 plana) sada je knjiga
sa slikama.

**Isti dan, `9087.pdf` (New In Chess):** font `NICRoest`, za koji mape nije
bilo — a gore od toga, mapa za SkakNew ga je „prepoznala" po 2 od 184 reda
dijagrama (slova se preklapaju), pa je knjiga išla putem fonta, tamo bila
odbijena, a put slika je nije ni video. Sada: mapa za NICRoest (pročitana sa
početne pozicije u samoj knjizi, 51 dijagram, svi proverljivi), mapa uzima
knjigu tek kad objasni bar polovinu redova dijagrama (pravi: 80–100%, lažni:
1%), a red koji knjiga štampa dvaput na istom mestu (NICRoest pa
NICRoest-Italic) čita se jednom. Svih osam ranijih knjiga daje bajt za bajt isti
rezultat.

**Otvoreno:** skakač na tamnom polju u Fritz fontu još nije viđen (tabla s
njim se glasno odbija); poruka „font we cannot read yet" sada znači da ni crtež
nije našao tablu. Detalji i brojevi: `PLAN-SKENER-SLIKE.md`, faza 3h.

## Materijal: pozicije, zadaci i zagonetke — jedan model, tri glagola — 23.9.2026, faze 0–5 u kodu, ostaje provera uživo (stavke 231–235)

Vlasnik: skener radi, ali se pozicija iz knjige u fontu i iz knjige sa slikama
obrađuju različito, a razlika treba da bude **svrha**, ne izvor; traži se
najjednostavniji model koji korisniku (trener, učenik, ili neko ko radi sam)
daje moćan alat bez zatrpavanja ekrana, pretragu po materijalu, i put do
učenika ili do sopstvenog vežbanja. Veliki reset nije zabranjen; **sav
postojeći materijal je proba i sme da se obriše, ništa postojeće ne mora da se
prilagođava.** Plan: **`PLAN-MATERIJAL.md`** (na engleskom), šest faza sa
kapijama; sprovodi ga vlasnik sa Opusom.

Šta je izmereno i šta odlučuje (detalji u planu, §2 i §3):

- **Oba puta skenera već upisuju u istu tabelu** (`custom_puzzles`); razlika
  je samo u tome sa čim tabla stiže. Motor za stranu na potezu (obe strane,
  „ubedljivo bolji potez") radi samo na ekranu Saved Positions, posle
  čuvanja, i čuva ocenu a ne potez — put slika ga nikad ne vidi.
- **Model: jedan red, tri stanja izvedena iz reda** — nesređena (pita se pre
  svakih vrata), pozicija (Study), zadatak (Study · Solve · Send); poreklo je
  čip, nikad grana. Sedam čipova ostaje (pozicija je jedno, zadatak drugo;
  red prelazi iz jednog u drugi kad dobije zadatak **na mestu**, ne kopijom —
  `PUT /exercises/:id` to na serveru već ume).
- **Niko ne može da reši svoj zadatak** — rešava se samo u domaćem. Faza 1:
  `POST /exercises/:id/attempt`, isti sudija, red u dnevniku pokušaja sa
  `source = 'own'`, kartica „My exercises" na Practise sa redom za ponavljanje.
- **Zagonetke iz „Review entire game"** su četvrta stvar bez sudije i bez
  zapisa (studio ih samo stavlja na tablu). Postaju Find zadaci sa `origin =
  'mistakes'` — isti oblik koji `homeworkFromArchive.js` već piše — a
  `puzzle_sets`, rute, repozitorijum, kopija na uređaju i sedmi čip se brišu.
- **Pozicije sačuvane sa table se ne sele** — revizija vođe posle merenja:
  prvi odgovor je rekao da „nikad ne mogu da postanu zadatak", a mogu, kopijom;
  sačuvana pozicija je **linija** (ceo PGN stabla) koju soba vraća na tablu,
  a zadatak nema liniju. Ako vlasnik i posle §2 hoće seobu, to je poseban plan.
- **Greška usput:** „Assign to student" na zadatku-partiji ide na
  `/assignments/custom`, koji ume samo Find — server ga uvek odbije (faza 0).

Redosled: 0 (dve sitnice) → 1 (rešavanje svojih) → 2 (strana i odgovor pre
čuvanja, oba skenera) → 3 (zadatak na mestu; Biblioteka upija Saved Positions:
filter po izvoru, „Needs attention", pretraga) → 4 (zagonetke iz partije su
zadaci) → 5 (svoji zadaci-partije se beleže) → 6 (provera uživo, stavke
231–235).

**Faza 0 u kodu (23.9.2026, Opus):** „Assign to student" na zadatku-partiji
otvara editor domaćeg sa tom jednom stavkom; Find ide pravo kao i pre; dijalog
nudi samo prihvaćene učenike. Kapija `test/assign_game_exercise_test.dart`,
obe mutacije crvene. Aplikacija 3876 → 3879 (osnovica izmerena u worktree-u,
sve kao u `CLAUDE.md`). Proverava se uživo u okviru stavke 231.

**Faza 1 u kodu (23.9.2026, Opus):** svoj Find zadatak se rešava sam —
„Solve" na kartici u Biblioteci i kartica „My exercises" na Practise (Tactics),
preko `POST /exercises/:id/attempt` i `GET /exercises/queue`; presuda servera
upisuje se kao `own` pokušaj, domaći se ne dira. Usput: `/api/puzzles/attempt`
sada odbija `own`, inače bi klijent mogao sam sebi da upiše „rešeno". Aplikacija
3879 → 3891, backend 1637 → 1656 / 1769 → 1788. **Traži restart backenda.**
Provera uživo: stavka 231.

**Faza 2 u kodu (23.9.2026, Opus):** na oba skenera „Suggest sides with the
engine" pre čuvanja — predlog strane i poteza ispod svake table bez strane,
„Set side", „Set side and answer", „Accept all confident" (samo sigurni); nova
kolona `custom_puzzles.solution_source` (`book` / `engine`). Usput dve greške
sa mastera: skener fonta na telefonu nije pokazivao nijednu tablu (mreža 0 px),
i `AdaptiveCardRows` nije ponovo merio karticu koja naraste. Ručno okretanje
strane na fontu sada skida oznaku za proveru, kao na slikama. Aplikacija 3891 →
3906, backend 1656 → 1660 / 1788 → 1794. **Traži restart backenda** (nova
kolona). Provera uživo: stavka 232, sa merenjem iz §7 plana.

**Faza 3 u kodu (23.9.2026, Opus):** „Make exercise" na skeniranoj poziciji
pravi zadatak **na mestu** (isti red, knjiga ostaje); Biblioteka je jedina
polica — izbor izvora i „Needs attention (n)" pod Exercises i Positions,
pretraga po knjizi, broju i zadatku, „Assign to student" samo uz prihvaćenog
učenika; **Saved Positions je obrisan** (ekran, ruta, `GET /scans/puzzles`), a
„View" posle skeniranja vodi u Biblioteku na tu knjigu. Usput: zaglavlje
Biblioteke na telefonu bilo je više od ekrana (na masteru), i priručnik je
navodio oznake obrisanog ekrana. Aplikacija 3906 → 3915, backend 1660 → 1664 /
1794 → 1799. Provera uživo: stavka 233.

**Faza 4 u kodu (23.9.2026, Opus):** „Review entire game" navodi nađene
zagonetke, a štiklirane čuva kao Find zadatke iz „mistakes" (odgovor motora,
ime partije kao izvor). **Puzzle sets su obrisani** — tabela `puzzle_sets` (na
vlasnikovo da, posle brojanja: 5 setova, 25 zagonetki), rute, kopija na
uređaju, sedmi čip i „puzzle mode" u Analizi. Tabelu je obrisao `initDB` kad
sam greškom na tri sekunde pokrenuo server (zapisano u `LESSONS.md`); server je
ugašen. Aplikacija 3915 → 3901, backend 1664 → 1658 / 1799 → 1794. Provera
uživo: stavka 234.

**Faza 5 u kodu (23.9.2026, Opus):** „Play" na kartici svog zadatka-partije;
kraj partije ide na `POST /exercises/:id/game-result` i sudi se kao domaći
(pravila, pa tablebase), a pokušaj se upisuje samo kad je nešto presudilo.
Sam igrač: „Play N moves" kaže „Played" i ne pominje trenera; ako tablebase ne
odgovori, kaže se da partija nije uračunata. Aplikacija 3901 → 3906, backend
1658 → 1667 / 1794 → 1803. **Traži restart backenda.** Provera uživo: stavka
235. **Time je `PLAN-MATERIJAL.md` izgrađen**; ostaje faza 6 — vlasnikova
provera uživo, stavke 231–235 u `TODO-provera.md`.

## Pozicija bez strane na potezu se ne koristi dok se ne pita — 23.9.2026, u kodu, ostaje provera uživo (stavka 230)

Vlasnik je pitao gde se sve pozicije koriste i da li negde prolazi pozicija bez
određene strane. FEN ne može da kaže „ne zna se", pa se takva pozicija čuva kao
„beli na potezu" sa oznakom `needs_review`. Pitali su samo Saved Positions
(otvaranje), a domaći zadatak ju je odbijao na serveru. **Rupe:** Biblioteka ju
je otvarala u Analysis, pravila od nje vežbu i dodavala je u tutorijal kao
„beli na potezu", a soba ju je stavljala na zajedničku tablu isto tako —
najgore je bilo „Make exercise", jer nova vežba nema oznaku pa bi mogla da
ode učeniku. Sada sva ta vrata idu kroz jedno mesto
(`side_to_move_gate.dart`, `settledFen`): pita „Who is to move?", odgovor se
čuva na poziciji (server prepisuje FEN), i tek onda ide dalje. Kartica u
Biblioteci počinje sa „Side to move not set".

## Dijagrami kao slike: deljena kalibracija — 23.9.2026, u kodu, ostaje provera uživo (stavka 229)

Vlasnik: kalibracija nije privatna stvar i deli se bez pitanja; isti fajl
(SHA-256) je ista knjiga. Kad je knjigu kalibrisalo više korisnika, ne bira se
jedna nego se **spajaju**: ista tabla od dva korisnika je jedna tabla sa dva
glasa, neslaganje odlučuje većina a nerešeno izbacuje tablu, i bira se najviše
8 tabli koje najviše dodaju. Ništa novo se ne čuva — svaki nalog ima svoju, a
deljena se računa iz ostalih pri svakom pitanju
(`GET /scans/calibrations/:hash/shared`), bez imena. Tuđa tabla stiže
neproverena („Set up by another user"), ne računa se i ne pamti dok je
korisnik ne potvrdi uz sliku. U tekstu aplikacije: „another user", ne
„trainer" (vlasnikova reč).

## Dijagrami kao slike: prvo kalibracija, pa strane — 23.9.2026, u kodu, ostaje provera uživo (stavka 228)

Vlasnik: za knjigu sa slikama prvo kalibracija, pa tek onda izbor strana;
postojeća kalibracija se preskače ali može da se ažurira; a vrstu knjige
aplikacija sama utvrđuje. Skener sada čim se izabere PDF pita server
(`POST /scans/kind`, 16 strana raspoređenih po celoj knjizi, prvo test fonta pa
traženje slika) — knjiga sa kalibracijom na nalogu se ne pita. Knjiga sa
slikama ide pravo u kalibraciju ili, ako je završena, na „Choose the pages to
read" (sa „Update the calibration"); posle čitanja „Other pages". Knjiga u
fontu i knjiga koju server nije mogao da pogleda idu kao ranije (strane pa
Scan).

## Dijagrami kao slike: kalibraciju bira trener — 23.9.2026, u kodu, ostaje provera uživo (stavka 227)

Vlasnik: tri predložene table su obično susedne i pokazuju iste figure, a one
koje fale su drugde u knjizi. Sada trener sam lista **celu knjigu** (20 strana
odjednom) i bira table, a tabela 12 figura × svetlo/tamno polje kaže šta je
viđeno (✓), šta se pogađa sa druge boje (≈) i čega nema (○). Čitanje čeka dok
svaka figura nije viđena bar na jednoj boji; „No … in this book" to preskače.
Ko je na potezu trener i dalje postavlja na svakoj tabli sam — LLM je na reč
vlasnika izbačen iz razmatranja.

Merenje 3e.0 (plan, faza 3e) pre gradnje: na Silmanu je stara šema u 8 od 11
opsega čitala bar jednu figuru koju kalibracija nikad nije videla, a 2–3 table
iz cele knjige pokrivaju svih 24 klasa. Nagoveštaji „možda pokazuje šta fali"
izmereni i **odbačeni** (u repu dodaju ništa, a čitanje knjige posle svake
table traje minutima); umesto toga server označava polje koje je dalje od
svakog primera od 10% (`UNKNOWN_INK`) — na skenu je to 32 neoznačena pogrešna
polja svelo na 4, na digitalnim knjigama nije dodalo nijednu oznaku.
...
Vlasnik je prihvatio skicu i sve tri preporuke:
- kalibracija je na **nalogu** (tabela `book_calibrations`, knjiga = SHA-256
  fajla, čuvaju se samo pozicije i njihovo mesto u knjizi);
- vrata se otvaraju **sama** kad sken fontom ne nađe tekst, a nađe slike;
- preko 60 tabli dobija **poruku**.

**Šta je napravljeno.**
- Server: `/scans/calibrations/:hash` (GET, PUT, DELETE), svaki upit sa
  `user_id`.
- Aplikacija:
  - vrata na Position Scanneru;
  - `ImageScanScreen` — tri table za postavljanje, svaka pored svoje slike;
    zapamćena kalibracija ide pravo na čitanje;
  - potvrda — slika pored pročitanog, nesigurno polje sa isprekidanim okvirom
    i „?", potez jednim dodirom, ispravka dodirom na tablu;
  - editor table ima opcionu sliku iz knjige (pored kontrola na širokom
    ekranu, iznad palete na telefonu).

**Nađeno usput i popravljeno.**
1. Kalibracija koja nije pročitala se ne pamti — inače bi se vraćala i padala
   pri svakoj poseti.
2. Table koje je trener postavio mogu da se sačuvaju sa ostalima.
3. Svi pozivi `ScannerApiService` idu kroz jedan klijent — čuvanje je prolazilo
   pored lažnog klijenta u testu.
4. `AppFeedback.dismiss` sklanja poruku odmah umesto animacijom. Animacija je
   tvrdila `mounted` posle nestanka messengera, van dometa `try`-a; isti kvar
   je imao i stari skener.

**Brojevi.** Aplikacija 3752 → **3778**. Server 1585 → 1596 bez baze, 1697 → 1709
sa bazom (faza 3a, oba izmerena).

## Dijagrami kao slike: faza 2 — čitanje sa kalibracijom — 22.9.2026, server u kodu, aplikacija ga još ne zove

**Dva merenja pre dizajna.**
- **Knjiga se ne čita šablonima druge knjige**: šabloni iz dve knjige čitaju
  treću — *Back to Basics* 51/52, Reinfeld samo 126/150 rešenja, Silman
  **0/24**.
- **Zato svaka knjiga ima kalibraciju**: trener da poziciju nekoliko njenih
  tabli, ostale se čitaju prema njima. Šabloni se ne čuvaju — bili bi isečeni
  iz knjige — pa kalibracija putuje sa zahtevom (`[{page, index, fen,
  ignore?}]`), a aplikacija šalje PDF ponovo.

**Šta je napravljeno.**
- `reader.mjs` (čitanje), `readWorker.mjs` (van glavne niti), `imageRead.mjs`
  (kalibracija, pregledi, predlog tabli za kalibraciju, odgovor).
- `POST /scans/images` — bez kalibracije vraća table, pregled svake i tri
  predloga; sa kalibracijom vraća pozicije sa `source: 'image'` i nesigurnim
  poljima. **Ništa se ne čuva**; čuva `POST /scans/confirm`, bez izmene.
- `POST /scans` na `no_text` / `no_diagram_text` sada kaže i
  `details.imageDiagrams` — vrata umesto zida.
- Najviše 60 tabli po zahtevu (~30 s u radnoj niti).

**Brzina.** Veran port faze 0 bio je 6,7 s po tabli. Svaka klasa sada sama
traži svoj pomak, a zatim se porede primeri, najviše 6 po klasi — **ispod pola
sekunde po tabli**, isti rezultat kao Python.

**Cela Node cev na vlasnikovim knjigama:**
- *Back to Basics* 51/52 (ista tabla kao Python), tiho pogrešnih 0, 1,5
  oznake po tabli;
- Reinfeld 291/300 rešenja;
- Silman 22/24, obe greške označene, 0,6 oznake po tabli.

**Tri nalaza koja su promenila kod.**
1. **Uvećanje table mora biti bilinearno.** Skenirana tabla je manja od 512
   piksela, a uvećanje 1-bitne slike u blokovima čini liniju debelom 1 ili 2
   piksela, zavisno od toga gde padne. Beli pešaci na svetlim poljima su se
   čitali kao prazni na 4 table od 24. „Inertna" mutacija iz faze 1 bila je
   inertna samo za *smanjenje*.
2. **Sastavljena klasa se nikad ne veruje.** Polje pročitano kao klasa koju
   kalibracija nije pokazala uvek se označava. Prva verzija je označavala i
   polja gde je sastavljena klasa bila *druga* — 20 oznaka po tabli, jer je
   sastavljen šablon uglavnom prazno polje.
3. **„Svako pogrešno polje je označeno" važi do pomaka od 6 px, ne dalje.** Na
   8 px 4 od 10 grešaka su bez oznake, sve ista: sastavljeni beli top na h1.
   Zato faza 3 traži još jednu kalibracionu tablu kad odgovor navede
   sastavljene klase.

Usput, merni alat je prvo prijavio *Back to Basics* 27/52 sa 25 tiho
pogrešnih. Kriv je bio alat: labele je vezivao po kutiji unutar slike, a
strana sa dva dijagrama ima istu kutiju u obe slike. Kropovi su prvo upoređeni
piksel po piksel (0,79 sive razlike), tek onda je išta u čitaču diran.

Server 1570 → **1585** bez baze, 1682 → **1697** sa bazom. Jedan pad u jednom
pokretanju sa bazom ostao je neobjašnjen i nije se ponovio u tri sledeća.

## Dijagrami kao slike: faza 1 — table isečene na serveru — 22.9.2026, u kodu, još bez rute

Vlasnik je 22.9.2026 prihvatio fazu 0, odmrzao plan (`PLAN-ZAVRSNICA.md`,
dodaci posle zamrzavanja) i pustio fazu 1; treća mapa fontova je u backlogu.

**Gde radi — izmereno:** na serveru, u Node-u, **bez nove zavisnosti**.
`pdfjs-dist` daje piksele slike bez platna (`getOperatorList` +
`page.objs`), 10–100 ms po strani. Skener fontova zadržava pravilo „bez
Pythona i bez ičega novog na serveru".

**Šta je napravljeno** (`services/positionScanner/`): `images.mjs` (slike strane
kao 8-bitna siva, sa mestom na strani, za svaku vrstu koju pdfjs vraća),
`boards.mjs` (okvir u slici dijagrama; table na skeniranoj strani — najveća
prva, bez zahteva da je okvir zatvoren, šahovnica kao provera), i
`imageDiagrams.mjs` (`findImageDiagrams`, plus komandna linija za poređenje).
Za rutu još nije vezano: faza 2 je prva koja ima čitanje da vrati.

**Paritet sa fazom 0 na vlasnikovim knjigama:** 391 / 1002 / 648 tabli —
iste kao faza 0, na svakoj strani; kutije identične do piksela za dve knjige
sa slikama, u granici 1,7 px za Silmana (faza 0 je čitala 200 dpi, Node čita
skenirane 300 dpi).

Kapija: 9 slučajeva u `imageDiagrams.test.mjs`, svi crteži nastaju u testu,
nijedna knjiga nije u repozitorijumu. Mutacija je našla dve rupe u **fiksturama**
(dupli okvir nacrtan unutra pa spojen sa šrafurom; tabela sa prekratkim
linijama) — obe popravljene, obe mutacije sada padaju na pravom slučaju; dve
su inertne i zapisane. Server 1561 → **1570** bez baze, 1673 → **1682** sa
bazom (oba izmerena).

## Dijagrami kao slike: faza 0 izmerena — 22.9.2026, ništa u aplikaciji ni na serveru, čeka odluku vlasnika

Na vlasnikov zahtev („kreni dalje po planu", knjige u `D:\chess\pdf_books` i
`D:\chess books`). Alat je u `tools/diagram_vision/`, brojevi u §7 plana
`PLAN-SKENER-SLIKE.md`; iz knjiga ništa nije u repozitorijumu.

**Rezultat:** *Back to Basics* 51/52 table bez greške, oba pogrešna polja
označena; Reinfeld (1001 žrtvovanje) — rešenje iz knjige se odigra do kraja na
978/995, a 16 nasumičnih tabli pregledanih polje po polje nemaju grešku; Silman
(ceo skeniran, bez teksta) 23/24 na skupu koji je vođa pročitao okom, pogrešno polje
označeno. **Tiho pogrešnih nije viđeno nijednom** — ali 0 od 76 ograničava
stvarnu stopu tek ispod ~4%, a prag iz plana je 1%; za to treba ~300 tabli sa
poznatom istinom. **To je odluka vlasnika**: graditi na ovome, ili prvo veći
skup.

**Bez neuronske mreže.** Svaka knjiga se čita šablonima iz 3–8 njenih tabli
koje je ovde označio vođa, okom — slika-put ima svoju „mapu" po knjizi, kao font-put po
azbuci. Isto podešavanje (binarizacija, najbliži primer, klizanje 10 px) važi
za sve tri knjige. Za fazu 3 to je dizajn: prve table koje trener potvrdi iz
nove knjige *jesu* njeni šabloni.

**Ostaje otvoreno:** oznake nesigurnosti nisu kalibrisane (1–11 po tabli, a u
svim skupovima istine su samo 3 pogrešna polja); skenirane table greše na
...
**Pitanje vlasnika:** da li skener PDF-a staviti na neki tab, i da li
omogućiti skeniranje knjiga bez šahovskog fonta (skenirane strane, dijagrami
kao slike) — ideja iz ranijeg razgovora sa Geminijem: nađi tablu, iseci 64
polja, klasifikuj ih malom mrežom, napiši FEN.

**Kartica „Scan a book" na Teach.** Do danas su jedina vrata bila u traci
Analize, na telefonu iza ⋮ — tamo je skener prešao 18.9. samo da brisanje reda
iznad table ne bi zatvorilo skener. Sada je treća kartica u redu „Preparation /
New session" (`teach_tab.dart`, ključ `teach-scan-card`), a vrata u Analizi
ostaju. `onOpenScanner` je **obavezan** parametar, ne opcioni: opciona vrata
su ona koja se jednog dana izostave. Test u `teach_tab_layout_test.dart`
proveren je dvaput na pogrešnom kodu — bez kartice (tri crvena, sva zato što
kartice nema) i sa karticom vezanom za Library (crveni tačno novi slučaj).

**Priručnik je već bio zastareo**: `analysis.html` i `getting-started.html`
su i dalje opisivali red „My games / Scan a book" pored table, ukinut 18.9.
Ispravljeno, i `getting-started` i `preparation` sada navode Teach.

**Dijagrami kao slike — `docs/PLAN-SKENER-SLIKE.md`, ništa u kodu.** Ideja je
dobra u smeru, ali nije „100% pouzdana": klasifikator je verovatnoćni, a
opasna greška nije kralj koji fali nego lovac pročitan kao pešak — legalna
pozicija koja izgleda tačno. Zato plan čuva odluku od 4.9.2026: poseban put sa
oznakom pouzdanosti po polju i obaveznom potvrdom; ko je na potezu, rokada i
en passant se ne pogađaju; potez iz rešenja je **provera, nikad popravka**; a
„popravljanje" OCR-ovanih poteza po legalnosti je van plana (pravilo 13).
**Faza 0 je samo merenje** (`tools/diagram_vision/`, ~80 ručno označenih
dijagrama iz *Back to Basics* i Reinfelda, gotov prepoznavač protiv malog
modela treniranog na sintetičkim dijagramima), a vlasniku predlaže prag za
gradnju: ≥ 90% tabli bez ijedne greške i ≤ 1% tiho pogrešnih. Plan dodaje
mogućnost, pa je ukidanje zamrzavanja iz `PLAN-ZAVRSNICA.md` odluka vlasnika.
Vektorski crtane table (*Grandmaster Codex*) su verovatno čitljive tačno, bez
verovatnoće — to je poseban, mali pogled, ne deo ovog plana.
...
su skrivena funkcionalnost: služe da AI iskomentariše poziciju ili partiju, i
računaju se samo kad se nešto šalje API-ju — nikad zato što se tabla promenila.
Uklonjeno iz onoga što čitalac vidi:

- paneli „Tactical motifs" i „Positional factors" u Analizi (i u listi panela —
  sheet se sad zove „Panels");
- automatski komentar na odigranom potezu, i sa njim podešavanje
  `manualCommentMode` („Comment new moves automatically");
- lista nalaza za štikliranje u uredniku komentara — urednik je sad običan tekst
  (postojeći `showCommentDialog`, koji do tada niko nije zvao);
- rečenice koje su Auto Analysis i „Review entire game" upisivali ispod poteza, i
  „Overwrite existing comments" u Review-u. Review i dalje stavlja oznake (?, ?!, !)
  i vraća `GameMoment`-e, iz kojih tutorijal čita nalaze.

Ostaje: „Generate AI comment" (Gemini, `/api/ai/generate-move-comment`) računa
nalaze za potez, prethodni potez, varijante i alternativu motora; tutorijal iz
partije (DeepSeek, `routes/gameTutorialWords.js`) ih čita iz momenata Review-a.

**Kapija.** `test/analysis_motifs_hidden_test.dart` — potez sa viljuškom ne dobija
komentar, nijedan panel motiva se ne crta ni ne nudi, i ekran Analize zove
detektore **samo** unutar `_findingsPairFor` (čitano po zagradama). Sva tri slučaja
viđena crvena na starom kodu, iz pravog razloga. Obrisani testovi obrisanog koda:
`analysis_mating_move_comment_test`, `core/mating_move_comment_test`,
`splitCommentForChecklist` i prekidač u `analysis_panels_sheet_test`. Pravilo
„ništa ispod mata" nije preneseno: ništa više ne piše komentar samo.
...
Vlasnikova proba sa dva naloga: nema glasa, snimanje „uz više prisutnih", nejasno
ko vodi. Log servera je pokazao jedan uzrok — **dva naloga su bila u dve različite
sobe** (stari poziv iz zvonca): soba se nikad ne završava, poziv nikad ne ističe,
a sačuvana „aktivna sesija" blokira svaku drugu sobu. Da li glas radi kad su oba
u istoj sobi **još nije viđeno**. Plan, dvanaest nalaza i vlasnikovih pet odgovora
su u [PLAN-SESIJA.md](PLAN-SESIJA.md): jedna živa sesija po treneru, jedan
voditelj, soba kaže ko je tu, snimanje izlazi iz sobe (5a) i vraća se u
Preparation (5b — oblik deljenja sa učenicima još otvoren), zakazane sesije i
prekidač za goste napolje, poziv samo iznutra.

**Faza 5b je u kodu** (22.9.2026, vođa): snimanje u Preparation — „Start recording" u traci, glas i tabla na satu samog zvuka (kao narracija; `LessonRecorder` i `audioTrimmer.js` obrisani), privatan zvuk kroz potpisan link, deljenje sa sopstvenim učenicima kroz prihvaćenu vezu (samo snimci iz Preparation), a učenik preuzima trenerov poslednji video dok postoji. Usput: film nikad nije pratio skokove sobnog snimka (`fen_change`/`lesson_loaded`), plejer je davao prednost ranijem `move` nad kasnijim `init` (popravljeno), `video_url` sa trenerovim tokenom više se ne predaje čitaocima, kartica snimka više ne piše „0.0 min". Stavke 219.37–43; aplikacija **3719**, server **1558 / 1667** (oba merena), analyze 26. **Otvoreno za vlasnika:** zvuk starih sobnih snimaka je i dalje javna `/uploads/` putanja; polica Library pokazuje snimke samo vlasniku (učenik deljeni snimak nalazi na Home → Recordings); film starih sobnih snimaka i dalje ne prati skokove; politika privatnosti §3.3. **Sledeće: faza 7 (proba na dva uređaja).**

**Faza 6 je u kodu** (22.9.2026, vođa): traka sobe nosi glas (ikona je stanje, dodir otvara panel „Voice"), trenerov panel „Session" (prisutni, poziv, „Students may move", okretanje učeničke table, „Room access"), ⋮ (Export to Analysis, Settings) i izlaz; desna kolona su potezi; učenik ispod table ima „Show my position to trainer" i tri brza odgovora, i bez glasa; na uspravnom telefonu naslov je samo rečenica o prisutnima, u dva reda. **Na vlasnikovu reč: učenik nema motor u sobi** — prekidač i oba događaja su obrisani, kolona `rooms.allow_student_engine` ostaje u šemi (brisanje je posebno „da"). „Mute all students" je obrisan, a podignuta ruka se spušta. Stavke 219.30–36; aplikacija **3711**, server **1552 / 1654**. Posle nje je došla faza 5b (gore).

**Faza 5a je u kodu** (22.9.2026, vođa): snimanje je izašlo iz sobe — nema kartice, nema `recording_*` događaja, nema `POST /recordings/save` ni provere spiska u sobi; `uploads/` i redovi `session_recordings` nisu dirani, plejer i MP4 izvoz rade na onome što postoji (stavke 219.28–29). Aplikacija **3681**, server **1550 / 1652** (oba merena), analyze 26. `LessonRecorder` i `audioTrimmer.js` čekaju fazu 5b, ništa ih ne pokreće do tada. **Otvoreno za vlasnika:** politika privatnosti (§3.3) i dalje kaže da korisnik sam u sobi sme da snima — opisuje više nego što aplikacija radi, što je bezbedan smer, ali je pravni tekst. Posle nje je došla faza 6 (gore).

**Faza 3 je u kodu** (jedan voditelj, prekidač „Students may move", bez unapređivanja; trener otvara sa belima dole — stavke 219.18–20; aplikacija 3650, server 1567). **Faza 4: serverski deo u kodu** (prisustvo drži sve utičnice jedne osobe, pa „Grant microphone" stiže učeniku u sobi — 219.21; server 1569 / 1671, 1671 je aritmetika, nije mereno). **Faza 4: aplikacijski deo je u kodu** (21.9.2026; aplikacija **3679**, analyze 26): glas koji ne krene kaže zašto, traka „*Ime* is in voice — Join voice" ispod trake sobe, ponovna najava glasa kad se sobna utičnica ponovo posadi, i mrtav mikrofon na Windowsu — stavke 219.22–27. **Uz to serverska ispravka nađena usput** (poslednji koji izađe iz glasa nije se objavljivao; kasni `disconnect` stare utičnice brisao je mesto koje je nova zauzela) — dokazana u kopiji van repozitorijuma i preneta na vlasnikovu reč, server ugašen; server **1575 / 1677, oba merena**. Sve od 21.9. je komitovano zajedno. Posle toga je došla faza 5a (gore). Pre toga — **faze 1 i 2 su u kodu, i tri odluke sa probe** (pozivaju se i ulaze samo svoji učenici; kôd sobe ne kuca niko — Home kaže „In a session now"; poziv celoj grupi; traka sobe kaže ko je tu — stavke 219.13–17; aplikacija 3641, server 1564 / 1666). **Sledeće: vlasnikova proba 219.13–17, pa faza 3.** Ranije — faza 1 (tada aplikacija 3617, server 1567 / 1667; vlasnik je 21.9. prošao 219 osim tačke 8, a njegova prijava uz tačku 2 je bila `audio_leave` koji izbacuje iz sobe — popravljeno, tačke 219.11–12): sesija se završava
(`services/roomLifecycle.js`, „End session" za vlasnika), nova gasi prethodnu,
završena soba ne prima nikoga pa ni glas, zvonce crta Join samo za živu sobu,
zapamćena mrtva soba se zaboravi sama, zakazane sesije i „Today" su obrisani
(dve tabele su ostale u bazi — brisanje je posebno „da"). Server traži restart
(nove kolone `rooms.created_at`, `ended_at`). **Sledeće: vlasnikova proba sa dva
uređaja (219, tačka 3 je glas), pa faza 2.** Deljenje snimka iz faze 5b:
odlučeno — samo u aplikaciji, po odnosu, sa mogućnošću preuzimanja; bez javnog linka.
...
## Domaći grupi, ista imena analiza, i ko podučava u sobi — 21.9.2026, u kodu, ostaje provera uživo

Tačke 9, 4 i 5 iz pregleda komentara, svaka po odluci vlasnika istog dana.
**Prva izmena servera danas**: nova ruta `PUT /analysis/:id`. Napisana i
merena u odvojenom worktree-u, izvan fascikle koju nodemon gleda (on gleda
svaki `.js`, i testove), pa je u radni direktorijum kopirana jednom, uz
vlasnikovo „da" — jedan restart, dok je on proveravao 215.

**9. Domaći celoj grupi** (prijava od 20.9). U dijalogu za slanje stoji čip za
svaku grupu; pritisak štiklira njene članove **kakvi su sada**. Vlasnik je
odlučio da učenik koji se kasnije učlani **ne** dobija domaći sam: svaka kopija
je jedinica kvote, rok i napomena pripadaju tom slanju, a pravilo koje se
kasnije samo okida je oblik greške koji ova baza plaća najčešće. **Rupa koju to
„ne" ostavlja, i kako je zatvorena:** server ne odbija drugu kopiju istog
domaćeg istom učeniku (nema jedinstvenog ključa), pa bi ponovno slanje grupi
posle novog člana svima ostalima dalo drugu kopiju i drugu jedinicu. Dijalog
zato čita `sent` samog domaćeg: ko ga već ima piše „already has it" i čip ga
preskače (ručno se može, namerno); ko nije prihvatio poziv se ne štiklira i to
piše imenom. Ako se domaći ne može pročitati, dijalog to kaže umesto da ćutke
ostane bez te zaštite. Server nije diran.

**4. Isto ime analize** (201.9). Server nikad ništa nije pisao preko — `POST`
samo dodaje — pa je isto ime pravilo drugi red koji izgleda isto kao prvi.
Jedina vrata za čuvanje (`promptSaveAnalysisDialog`, i Analysis i Preparation)
sada pitaju „Cancel / Keep both / Replace" kad ime postoji (poredjenje bez
razmaka na krajevima i bez obzira na velika slova); „Replace" ide na novu rutu
`PUT /analysis/:id`, zaključanu po `user_id` **i** `id` — id analize je mali
broj i sam nikad ne sme da dohvati tudji red. Lista koja ne može da se pročita
ostavlja staro ponašanje (nov red) umesto da blokira čuvanje.

**5. Ko ima alate za podučavanje u sobi** (201.10, „obojica imaju panel").
**Nadjeno pre gradnje:** sedište ne kaže ko je trener — server onome ko otvori
sobu daje `trener`, a svakom ko udje `ucenik`, pa u sobi koju je otvorio učenik
učenik sedi kao trener; a na Windows-u levu kolonu crta svakome. Vlasnik je
odlučio: **po odnosu**. `mayTeachInRoom` (`board_control_rules.dart`, pored
`canDriveSharedBoard`): alati za podučavanje („Make exercise", akcije
tutorijala u listi) pripadaju onome ko je prihvaćeni trener bar jednog drugog u
sobi, **ili** je otvorio sobu i jeste nečiji trener (da trener koji sprema pre
dolaska učenika ne ostane bez njih). Alati table i čuvanje sopstvene kopije
(Set up, Import/Export PGN, Save position, Save analysis, FEN) ostaju svakome ko
sme da pomera tablu. Proveren je i vlasnikov krug A → B → C → A i A,C → B:
server ne dozvoljava da dvoje budu jedno drugom treneri (proverava obrnutu
ivicu), trougao dozvoljava, i to ne smeta — **nijedno pravo u kodu ne ide
tranzitivno** (nema rekurzivnog upita ni spajanja odnosa na odnos), pa u krugu
svako podučava onoga koga zaista uči i ništa preko sredine. „Otvorio sobu" se
čita iz **prvog** sedišta koje server pošalje; granica, zapisana u kodu:
unapredjen član koji se ponovo poveže takodje dobija `trener` bez `changed`, i
čita se kao da je otvorio sobu — to ne košta ništa, jer ti alati pišu samo u
sopstvenu biblioteku. Tačan odgovor ima samo server (`seat.role === 'trener'`
samo za tvorca); da se prenese, trebalo bi dirati `server.js`.

**Tekst stavke 201.10 nije ispravljen** iako je pogrešno tvrdio da učenik
panel nema: odgovorena stavka se ne sme menjati (`spoji.py` je prepoznaje po
tekstu), pa je ispravka u stavci 217.

**Kapije i mutacije.** Grupa: `homework_send_group_test` (6; na master-u 4
crvena / 2 zelena), 6 mutacija; jedna je crvena samo na jednom slučaju jer
`_send` i sam šalje samo prihvaćenima — druga zaštita iza ove, zapisano.
Ime: `analysis_save_same_name_test` (7; 6/1) i server
`analysis_replace_routes.test.js` (5, svi crveni dok ruta nije postojala), 5
mutacija u aplikaciji i 3 na serveru; jedna je prvo pokvarila prevodjenje i
ponovljena je ispravno.
Soba: 8 slučajeva pravila u `board_control_rules_test` i
`room_teaching_tools_test` (5) na **pravoj** sobi — proba je pokazala da se
soba sa pravim kodom može podići i srušiti u testu, i ponovila prijavu
(učeničko sedište na širokom prozoru je imalo „Make exercise"); na master-u 8
crvenih. 7 mutacija; **jedna je preživela** (samoisključenje iz sopstvenih
učenika), jer slučaj nije dirao granu „otvorio sobu" gde ono jedino nešto znači
— sada dira. Test sobe ne može da dohvati spisak prisutnih (stiže preko
Socket.IO), pa „trener nekog prisutnog" drže samo slučajevi pravila; zapisano u
zaglavlju testa. **Dve prelivanja u desnoj koloni sobe** u probi su bila od
test-fonta, ne od sobe (pravilo 8): sa pravim Roboto nema nijednog.

Mereno: aplikacija **3566 → 3592**, 1 preskočen; `flutter analyze` istih 26
`info`. Backend **1545 → 1550** bez baze i **1639 → 1644** sa jednokratnim
klasterom, mereno u worktree-u sa istim fajlovima koji su posle kopirani.
Uživo: **stavka 217**.
...
## Brisanje iz Biblioteke, „Make exercise" na poziciji, i oznake i jezik na telefonu — 21.9.2026, u kodu, ostaje provera uživo

Tačke 3, 6 i 7 iz pregleda komentara (odeljak ispod). Samo aplikacija.

**3. Set zagonetki i analiza se brišu sa police** (211.4, „Nema dugme za
brisanje"). Kartice tih vrsta nisu imale nijedno dugme; set se brisao samo iz
Analize → „Saved puzzle sets", mesta za koje je vlasnik već rekao da ne zna
gde je. Obe rute na serveru su postojale. Sada kanta, pitanje, pa zahtev.

**Usput nadjeno, i važnije od dugmeta:** `PuzzleSetRepository.delete` je
brisao kopiju sa uredjaja **bez obzira na odgovor servera**, pa je odbijeno
brisanje izgledalo obavljeno, a set se vraćao pri sledećem učitavanju — oblik
greške koji ova baza plaća najčešće. Sada se kopija sa uredjaja briše tek kad
je nalog pustio set, `delete` vraća `bool`, a oba mesta koja brišu (polica i
dijalog u Analizi) na odbijanje kažu to i ostave karticu. Rečenica
(`notDeletedMessage`) je u `app_feedback.dart`, jedna za sva vrata.

**7. Pozicija se ne šalje, već se prvo pravi zadatak** (205.3; pravilo vlasnika
od 19.9, 185.5). Od faze 10 sken bez rešenja je pozicija, a kartica mu je i
dalje nudila „Assign to student" — vrata koja server može samo da odbije.
Kartica sada pita šta stavka **jeste** (`isExercise`), ne iz koje je tabele:
zadatak ima „Assign", pozicija (sken bez rešenja i pozicija iz sobe) ima
**„Make exercise"** — isti `MakeExerciseSheet` koji otvara soba, na toj
poziciji bez odigranog poteza, pa „Find the move" vodi na ekran zadatka
(„Play the move", faza 14). To su i vrata koja je tražilo vlasnikovo pitanje 5
(„sken → zadatak"), bez samostalnog rešavanja, koje ostaje novi plan.
Traka za višestruki izbor u skeneru (`saved_positions_screen`) i dalje ima
„Assign" — tamo server odbija sken bez rešenja i dijalog imenuje svako
odbijanje, pa je iskreno; ostavljeno.

**6. Oznake i jezik tutorijala na telefonu** (prijava od 20.9.2026). Raspored za
telefon ih je namerno izostavio u fazi 6b, pa tutorijal napravljen na telefonu
nije imao po čemu da se nadje ni jezik za glas. Sada ⋮ „More" → **„Details…"**
otvara list sa **istim** `_labelsField` i `_languageField` koje crta desktop —
isti kontroler, nijedna kopija pravila. List sluša `_c` sam, jer ga
`setState` ekrana ne gradi ponovo; bez toga bi padajući meni pokazivao jezik sa
kojim je otvoren (mutacija M8 to pokazuje).

**Kapije.** `test/library_card_doors_test.dart` (7), `test/
tutorial_phone_details_test.dart` (3, uspravno i položeno, tvrdnja na zahtevu
za čuvanje) i novi slučaj u `puzzle_sets_sync_test.dart`. Na master-u **10
crvenih / 1 zelen** — zeleni je „zadatak zadržava Assign", koji mora da važi i
pre; slučaj u `puzzle_sets_sync_test` je prvo pokazan crven po ponašanju
(„the device copy went"), bez provere povratne vrednosti, jer se `void` ne
prevodi — to bi bila crvena iz pogrešnog razloga. **Deset mutacija, svaka
crvena na tačnom slučaju.**

Pun prolaz je oborio jedan postojeći slučaj, `library_grid_3b_test` („a card
still carries its own kind's actions"), koji je tvrdio „an analysis has no
actions and must draw none" — tačno pravilo koje je tačka 3 promenila na
vlasnikov zahtev. Prepisan **otvoreno**, sa razlogom iznad njega, i ne
olabavljen: analiza crta tačno jedno dugme, i to brisanje (mutacija koja ga
skine je crvena). Susedni slučaj koji meri da sadržaj kartice analize stoji na
vrhu ostao je isti; ispravljen mu je samo tekst koji je tvrdio „no actions".

Mereno: aplikacija **3546 → 3557**, 1 preskočen; `flutter analyze` istih 26
`info`. Uživo: **stavka 215**.
...
u aplikaciji; server se ne dira.

**1. „Resume analysis" je i dalje nudio tudju analizu** — prijava od
20.9.2026, dva dana posle popravke od 18.9 (stavka 177.2). Ta popravka radi:
odjava briše skicu. Ono što nije videla je **redosled na izlazu**: `signOut()`
briše, *pa* `context.go(login)` ruši ljusku, i `dispose` ekrana Analize
upisuje stablo koje još drži (`flush`). U tom trenutku uredjaj pripada gostu, a
gostov rad se predaje onome ko se sledeći prijavi — pa je brisanje poništeno
kadar posle. Uzrok je nadjen čitanjem, a test ga je reprodukovao na master-u
tačno tim redom („account 22 was handed account 11's tree").

Popravka je jedan broj u `AccountLocalState`: **`epoch`**, koji se pomera svaki
put kad `clear()` briše tudje stanje. Pisac (ekran Analize, kontroler
tutorijala) uzme broj **kad nastane** i predaje ga uz svaki upis; upis pisca
starijeg od poslednjeg brisanja se odbacuje (`isCurrent`). Uzima se pri
rodjenju, ne pri upisu — pri upisu je broj uvek tekući, i to je baš greška
(mutacija M1 to pokazuje). `scheduleSave` i `flush` oba servisa traže `epoch`
kao **obavezan** parametar, iz istog razloga kao repozitorijum zagonetki: opcioni
bi neko izostavio. Studio tutorijala ima isti `flush` u `dispose`, pa je
ogradjen istim pravilom. Gost koji se prijavi i dalje zadržava svoj rad
(nema brisanja, pa se broj ne pomera).

**Čip „Resume analysis" je uklonjen** na vlasnikovu reč: Analyse tab se ionako
otvara na stablu na kom je ostavljen, pa su to bila druga vrata na isto mesto.
**Samo uklanjanje čipa ne bi popravilo prijavu** — tab sam učitava skicu pri
otvaranju (`_restoreDraft`), pa bi tudje stablo stajalo na tabli bez čipa.
„Resume session" (živa soba) ostaje.

**2. Motor staje kad se ode sa ekrana, i ostaje ugašen.** Analyse živi u
`IndexedStack`-u ljuske, koji skrivenom tabu čuva stanje *i tikere* — samo ga
ne crta — pa ekran nikad nije saznao da je napušten i motor je računao za
tablu koju niko ne gleda. Isto kad se preko Analize otvori ceo ekran. Jedan
signal pokriva oba izlaza: **`TickerMode`**. Ljuska sada omotava svaki tab u
`TickerMode(enabled: i == _selectedIndex)`, a `Overlay` to već radi za rutu
prekrivenu neprozirnom. Ekran sluša `TickerMode.getValuesNotifier` (stari
`getNotifier` je zastareo i dao bi nov `info`). Pri odlasku gasi oba prekidača
i **pušta** motor (`detach`) umesto `stopAnalysis`: ekran otvoren preko Analize
možda koristi isti motor, a `detach` zaustavi samo ovu pretragu i vrati motor
onome ko je na vrhu. Pri povratku ga opet uzme, sa oba prekidača ugašena.
Dijalog i list nisu odlazak — tabla je i dalje pred čitaocem.

**Kapije.** `test/left_behind_on_sign_out_test.dart` (4 slučaja: Analiza i
tutorijal posle odjave, i gost za oba) i
`test/analysis_engine_off_when_left_test.dart` (2: ruta preko Analize, promena
taba). Na master-u **4 crvena / 2 zelena**, svaki crveni iz pravog razloga.
`home_tabs_test` je dobio slučaj da čipa nema, sa pravom skicom na disku.
**Deset mutacija, svaka crvena na tačnom slučaju.** Jedna je prvo **našla rupu
u kapiji**: sa svim upisima odbijenim („gost" slučaj za Analizu) ostao je
zelen, jer je čitao nazad fixture — skica je bila na disku pre ekrana. Sada se
skica skine sa diska čim je ekran učita, pa je jedini put da bude tu na kraju
upis samog ekrana; ista mutacija je sada crvena (pravilo 6).
...
- ~~ista imena analiza (201.9), panel u sobi koju je otvorio učenik (201.10),
  slanje domaćeg grupi~~ — **u kodu 21.9.2026**, odeljak iznad, stavka 217.
- *Čeka vlasnika:* pretraga po datumu u „Choose a game" (predlog: sortiranje
  klikom na kolonu Date); bedž na Teach a red za pregled na Home (otvoreno od
  18.9).
- *Neviđeno uživo:* 193.3–4, 198.2–6, 209.4/5/7/8/10.

---

## Početni tabovi: faze 3 i 4 — 21.9.2026, ✅ provereno uživo istog dana

**Zatvoreno.** Vlasnik je 21.9.2026 prošao stavke 212 i 213 u celosti i
potvrdio ih. U istom prolazu je rekao da se kartica „Chess trainer and drills"
na vrhu Practise-a ukloni (plan §6 ju je nudio kao kandidata) — uklonjena je,
a slučaj „the phases are the first thing on the tab" pada ako se iznad faza
išta vrati (meri položaj, ne traži reči; na staroj kartici crven sa 262 px). Aplikacija **3538 → 3539**, 1 preskočen.
Uz to je okvir testa Practise-a ispravljen da drži hub u vertikalnom
skrolu kao `TrainingHubScreen` — bez toga je `Center` hub centrirao po visini
prozora od 3000 px i novi slučaj je bio crven iz pogrešnog razloga.


Vlasnik je 21.9.2026 pogledao Teach (faza 2) u širokom prozoru i na telefonu,
potvrdio raspored i premeštanje Library ispod Homework, i odobrio faze 3 i 4.
Time je **`PLAN-POCETNI-TABOVI.md` odgradjen u celosti**; ostaje faza 5,
njegova provera uživo (stavka 213, i tačke 2 i 3 stavke 212).

- **Faza 3: Home.** Granica od 700 px je nestala. Set for me · Due for review ·
  Join a session u jednom toku (`AdaptiveCardRows`); redovi Trainer panela
  teku u kolone ispod svakog naslova; snimci su kartice sa okvirom u koloni.
  Red panela je izgubio sopstveni donji razmak — razmak daje tok, pa su redovi
  12 px razmaknuti umesto 8, i na telefonu. Kapija
  `test/home_tab_layout_test.dart`, 6 slučajeva; na master-u 3 crvena i 3
  zelena (jedan crveni je nedostajući ključ toka, ne tvrdnja). 5 mutacija,
  svaka crvena na tačnom slučaju.
- **Faza 4: Practise.** Jedna kolona: redosled kao danas; dve: podela kao
  danas (Opening + Tactics │ Endgame); tri: po jedna za svaku fazu. Broj kolona
  iz `columnsFor` na širini koju kartice dobiju, nikad iz prozora
  (`Breakpoints.isWide` je izbačen iz fajla). Granica `maxColumns = 3` i širina
  izvedena iz nje, pa se broj i granica ne mogu razići. Razmak izmedju kolona
  je 12 umesto 20, jer ga `columnsFor` pretpostavlja. Kapija
  `test/training_hub_layout_test.dart`, 7 slučajeva; na master-u 4 crvena i 3
  zelena, svaki crveni tvrdnja. 5 mutacija, svaka crvena na tačnom
  slučaju.
- **Kapija plana je ispravljena pre pisanja:** plan je tražio dve kolone na
  1000 px, a `columnsFor` daje tri već od kutije od 865. Na prozoru od 900 px
  (najmanji na Windowsu) tab je širok oko 807, pa Practise tamo ima dve kolone;
  tri počinju oko prozora od 1000.

Aplikacija **3525 → 3538**, analyze istih 26 `info`. Uživo: **stavka 213**.

---

## Početni tabovi: plan, faze 0–2 — 21.9.2026, ✅ Teach provereno uživo 21.9.2026

Vlasnik je tražio da Home, Practise i Teach dobiju isti tretman kao liste u
`PLAN-LISTE.md`. Plan: **`docs/PLAN-POCETNI-TABOVI.md`**. Gemini-jev predlog
(`sugestije_layout_ostali_tabovi.html`, van repozitorijuma) je pročitan i
**nije** usvojen: fiksna tabla od 3 pa 2 modula je broj kolona zapisan rukom,
a red Trainer panela pretvoren u visoku karticu vraća većinu dobitka.

Vlasnikove tri odluke istog dana: **odluka 2 iz `PLAN-LISTE.md` se ukida**
(Home i Teach gube `maxWidth: 700`); Practise dobija po jednu kolonu za svaku
fazu partije, sa jednom izrečenom granicom od tri kolone; na Teach-u obrazac
za zahtev stoji **pored** spiska ljudi. Princip: *sekcije stoje jedna ispod
druge, njihov sadržaj teče u kolone* — broj kolona iz
`AdaptiveCardGrid.columnsFor`, iz širine koju sekcija dobije, nikad iz prozora.

- **Faza 0:** osnova izmerena u čistom worktree-ju: **3507** prošlo, 1
  preskočen, **26** `info`. (Prvi pokušaj je pokrenut u radnom direktorijumu
  dok su se menjali fajlovi u `lib/` — `flutter test` prevodi svaki test fajl
  kad dodje na red, pa bi izmene procurele u osnovu. Prekinut i ponovljen.)
- **Faza 1: `AdaptiveCardRows`** u `widgets/adaptive_card_grid.dart` — treća
  polovina obrasca A: kartice red po red, svaka u redu visoka kao najviša u
  njemu. Sopstveni render objekat, ne `IntrinsicHeight`: intrinsics bacaju
  izuzetak na `LayoutBuilder`, a ovi rasporedi se gnezde. 10 slučajeva, 6
  mutacija, svaka crvena na tačnom slučaju. Deseti slučaj drži da upit za
  intrinsičnu visinu pada glasno umesto da tiho vrati 0.
- **Faza 2: Teach.** Tutorials · Homework · Library, pa Preparation · New
  session, pa kartica ljudi (obrazac │ spisak, redovi teku). Obe kartice
  izgubile su sopstveni donji razmak — razmak u jednoj kartici reda bi je
  završio kraće od suseda. Kapija `test/teach_tab_layout_test.dart`, 8
  slučajeva; na master-u 6 crvenih i 2 zelena (dva crvena su nedostajući
  ključ obrasca, ne tvrdnja — zapisano). 3 mutacije, sve crvene na tačnom
  slučaju; jedna prva verzija je bila **nevažeća** (neuparene zagrade → ne
  prevodi se), ponovljena kao sužavanje koje se prevodi.
  **Jedna promena koju vlasnik nije tražio:** na telefonu je Library sada
  odmah ispod Homework. Prva tačka provere uživo; vraća se u jednom redu.

Aplikacija **3507 → 3525**, analyze istih 26 `info`. Uživo: **stavka 212**.
Sledeće: faza 3 (Home) i faza 4 (Practise).
...
## Liste: faza 7 — 21.9.2026, u kodu, ostaje provera uživo

„Choose a game" je postao prava tabela, na vlasnikov zahtev („sirovi tekst i
kartice mi i dalje bodu oči"). **Time je `PLAN-LISTE.md` odgradjen u celosti**
— ostaje samo faza 8, njegov prolaz uživo.

Iznad širine dijaloga od 560: zaglavlje koje stoji i virtuelizovane vrste od
**38 px** sa pet kolona — White · Black · Date · Result · First moves. Ispod
te širine (telefon): iste činjenice u dve zbijene linije, po §3.3 maketa.
Filter po rezultatu (All · 1-0 · ½-½ · 0-1) je ispod pretrage i **sužava ono
što je pretraga ostavila**, ne zamenjuje je. Dijalog je dobio više mesta:
640 → 900 široko, 640 → 720 visoko.

**Dve stvari iz maketa namerno nisu napravljene**, i razlozi su u kapiji:
kolona „Izaberi" (vrsta je već meta; jedna radnja ne traži dvoja vrata) i
paginacija `1 / 206` (`ListView.builder` virtuelizuje, pa bi stranice vratile
čitaoca na traženje broja strane). Filter je po **rezultatu**, ne po
„pobede/porazi" kako maketa traži, jer bi ovo drugo moralo da pogadja kojom je
stranom ovaj nalog igrao.

Kapija: 9 slučajeva, **7 crvenih / 2 zelena** na master-u, svi crveni tvrdnje
(tri su prepravljena iz `StateError` u tvrdnju). Nosilac faze nije „naslovi su
na ekranu" nego **ćelije stoje ispod svojih naslova**, mereno tamo gde se
crtaju i **u okviru jedne vrste** — „1-0" je na ekranu dvaput, kao čip i kao
rezultat, pa bi neograničen finder merio čip naspram kolone.

**Jedna mutacija je preživela prvi krug i kriva je bila fikstura, ne kod**:
slučaj kompozicije je pretraživao „pvladan", što su sve tri partije u fiksturi
zadovoljavale, pa filter koji potpuno ignoriše pretragu daje isti odgovor.
...
`BoardPreviewDialog` izvučeno je u `BoardPreviewPanel` i oba ga crtaju — jedan
dom za to šta „veći pogled na ovu stvar" uopšte jeste. `LibraryList` je dobio
**opcione** `onSelect` i `selectedId`, pa kolona u sobi nije ni dodirnuta
(pravilo 15): bez njih sličica otvara dijalog kao i do sada. Izbor se crta
**okvirom**, ne bojom. `selectedId` nosi i vrstu (`LibraryList.idOf`), jer
ideovi dolaze iz raznih tabela i pozicija 12 i tutorijal 12 oboje postoje —
mutacija je to dokazala pošto je fikstura dobila baš taj par.
...
Kapija: 9 slučajeva, **3 crvena / 6 zelenih** na master-u, sva tri crvena su
tvrdnje. Sedam mutacija; šest crvenih na tačnom slučaju, **jedna preživela** —
zamena ograničenja `MediaQuery`-jem ne menja ništa jer je `LibraryScreen` uvek
cela ruta, pa je granica zapisana u kapiji umesto da se izmišlja slučaj.
Mutacija koja isprazni deljeni panel obara **dijalogov** slučaj, što je dokaz
da izdvajanje nosi teret a nije kopija.

Aplikacija **3463 → 3474**, analyze istih 26 `info`, nijedan iz izmenjenih
fajlova. Bez izmena na serveru. Uživo: **stavka 208**.

---

## Vlasnikova provera 20.9.2026 uveče: tri nalaza — u kodu, ostaje provera uživo

Tri stvari nadjene uživo iste večeri (stavke 203–206), sve tri u kodu istu noć.
Uživo: **stavka 207**. Jedina od njih koja dira server je druga, pa **backend
treba restartovati** pre provere.

**A. Zagonetke iz Biblioteke se otvaraju.** Kartica „Puzzle sets" je stajala na
polici i na kucanje nije radila ništa — i to je pisalo u kodu, u komentaru:
u režim zagonetki se ulazilo samo kroz dijalog na ekranu Analize na kom su
izvučene. Vlasnik je istu grešku prijavio dvaput iste večeri: jednom kao mrtav
dodir, jednom kao „ne znam gde je to uopšte". `AnalysisStudioScreen` dobija
`initialPuzzles`, četvrti parametar oblika „otvori baš ovo", sa istim pravilom
kao ostala tri — pobedjuje skicu na uredjaju. Kapija: 6 slučajeva, **4 crvena /
1 zelen** na master-u, 4 mutacije.

**B. Brojevi u „What to drill" prate prekidač.** Najozbiljniji od trojeg, jer
nije bio mrtav prekidač nego **pogrešan broj**: katalog je brojao i online
partije, trening ih po podrazumevanom podešavanju ne servira, pa je „Selected:
N positions" bio veći od onoga što se može dobiti. Puna priča je u
`LESSONS.md`; kratko: dve rute nad istom tabelom odgovaraju na „koliko ih ima"
i „daj mi jednu" i moraju da broje isti skup. Server dobija istu liniju koju
druge dve rute već nose (`excludeOnlineClause`), `fetchCatalog` dobija
`includeOnline`, prekidač ponovo pita server, a kvačice čitaoca preživljavaju
taj refetch po zapisanom pravilu. Kapije: server **3 crvena / 3 zelena**,
aplikacija **4 crvena / 1 zelen**, šest mutacija ukupno.

**C. ☰ u sobi se lakše pogadja.** „Radi, ali je u takvom delu ekrana, da jedva
odgovara na pritisak." Izmereno pre izmene: uspravno 48 × 48 na (4, 4), položeno
48 × **44** na (4, **0**) — dakle veličina je već punila traku, a mesto je bilo
sam ugao. Dugme se sada crta ručno, uvučeno za onoliko koliko je sistem zauzeo
(`systemGestureInsets` / `padding`), sa podom od 8. **Da je krivac Androidova
traka za gest „nazad" je pretpostavka, ne merenje** — potvrdjuje se samo na
telefonu. Kapija: 5 slučajeva, 2 crvena na master-u, 3 mutacije, od kojih je
jedna **preživela i našla rupu**: pomeriti dugme bez širenja slota znači
stisnuti ga na 8 px, a sve ostalo ostaje zeleno. Za to sada postoji slučaj.

Mereno: aplikacija **3445 → 3463**, 1 preskočen. Backend **1530 → 1536** bez
baze, **1624 → 1630** sa jednokratnim klasterom, sve zeleno. Analyze: istih 26
`info`, nijedan iz izmenjenih fajlova.

Pri punom backend prolazu na master-u pao je `tutorial_preview_frames.test.js`
(„a preview leaves nothing in exports/", 53 !== 54); sam fajl je zelen i greška
se nije ponovila. Trka nad deljenim `exports/` dok `node --test` vrti fajlove
uporedo — **nije regresija, ali jeste test koji nije izolovan**, i vredi ga
srediti kad dodje red.

---

## Liste: faza 4 — 20.9.2026, u kodu, ostaje provera uživo

„What to drill" (`endgame_picker_screen.dart`): porodice završnica više ne
stoje u jednoj koloni nego se dele po širini. Nivoi i dva prekidača ostaju
jedno zaglavlje preko cele širine — to je pitanje postavljeno celom katalogu.

**Nalaz faze je da obrazac A ima dve polovine.** `AdaptiveCardGrid` je
sliver-delegat sa jednim `mainAxisExtent`, dakle list ćelija **jednake**
visine. Kartica porodice je jedan red dok je zatvorena i oko 800 px kad se
otvori (topovske završnice imaju trinaest oblika), pa jedna visina ćelije ima
samo dva ishoda i oba su greške koje je ovaj plan već platio: otvorena se
preliva, ili svaka zatvorena dobije visinu otvorene i prozor se puni vazduhom
— doslovno pritužba zbog koje je faza 3a dopunjena. Zato je u isti fajl stao
`AdaptiveCardColumns`: kartice se dele naizmenično u obične `Column`-e, svaka
kolona uzima visinu koja joj treba. Dobit nije samo da se ništa ne preliva —
**otvaranje jedne porodice ne pomera ostale**, dok je u `ListView`-u dodir na
topovske gurao pešačke 700 px naniže.

Broj kolona se i dalje nigde ne piše, ali se sada računa na dva mesta, pa je
formula izvučena u `AdaptiveCardGrid.columnsFor` i slučaj u testu mreže čita
odgovor delegata sa ekrana i drži pomoćnik uz njega — na granicama pojaseva
(431, 432, 433, 863, 864), što je jedino uhvatilo mutaciju „zaboravi razmak".

Kapija: **7 slučajeva, 4 crvena i 3 zelena na master-u**, sva četiri crvena su
tvrdnje. Sedam mutacija, svaka crvena na tačnom slučaju, **nijedna nije
preživela**. Kapija je prvo bila crvena iz tri pogrešna razloga (fikstura
`Map<String, Object>` koju `whereType<Map<String, dynamic>>` baci; strelica
kucnuta naslepo, što je prekidač, a ekran sam otvara najveću porodicu;
`find.ancestor(...).first` koji pukne umesto da padne kad `ListView` ne
izgradi ono ispod ivice) — sve tri su u `LESSONS.md`.

Aplikacija **3437 → 3445**, analyze istih 26 `info`, nijedan iz dva izmenjena
fajla. Uživo: **stavka 206**.
...
Vlasnikova ispravka je srž svega: *„kad sam rekao da ima dosta prostora na
širokom ekranu, nisam mislio da se to smanji ograničavanjem širine stavke, već
da se taj prostor iskoristi."* Time otpada i §2.4 Geminijevog dokumenta
(`max-width`) i **moj raniji odgovor na stavku 5 u `zadaci.md`**, koji je
predlagao zajednički `CenteredPane` sa istim ograničenjem. Oba menjaju jednu
prazninu za drugu: prozor od 1920 px sužen na 700 prikazuje isti broj stavki
kao i pre, popravlja se samo dužina reda.

Mehanizam, izmeren 20.9.2026: svaka od ovih listi je `ListView` → `Card` →
`ListTile`, a `ListTile` ima tri pregrade koje **ne rastu sa širinom** — raste
razmak između njih. Otuda „naslov, okean, ikonica". Sužavanje smanjuje okean;
drugu stavku u red ne može da doda. Uz to, `theme/breakpoints.dart` već ima
rečnik (`wide = 840`, `ultraWide = 1200`) koji koristi četrnaest fajlova — **ni
jedan od njih nije ekran sa listom.**

Tri šablona, svaki definisan jednom (ne šest zasebnih dizajna): **A** — mreža
sa maksimalnom širinom pločice (`SliverGridDelegateWithMaxCrossAxisExtent`,
420), pa broj kolona nikad nije upisan; **B** — master–detail za Library i
Repertoire; **C** — guste vrste + pretraga, za „Choose a game" (4126 vrsta u
kutiji 400 × 300 bez pretrage) i „What to drill".

Vlasnikove odluke 20.9.2026: tri Home taba **zadržavaju** `maxWidth: 700` (to
su stekovi kartica, ne liste — van opsega); **šablon B se radi do kraja**, jer
je to ono što aplikaciju čini desktop aplikacijom a ne razvučenim telefonom;
pretraga u „Choose a game" ide prva i sama.

Devet faza (0 merenje → 1 pretraga → 2 `AdaptiveCardGrid` → 3 šablon A → 4
„What to drill" u kolonama → 5 i 6 šablon B → 7 tabela → 8 provera uživo).
**Nema izmene na serveru ni u šemi** — sve je `chess_app/lib` i testovi, pa
ništa od ovoga ne može da zaustavi vlasnikov nodemon. Glavni trošak nije
raspored nego **površina testova**: ovi ekrani se pronalaze preko
`find.byType(ListTile)`, pa svaki brief od faze 3 imenuje koji se finderi smeju
menjati — tiho prepisana tvrdnja je upravo ono što zeleni paket čini
bezvrednim.

---

## Spajanje tutorijala i izdvajanje delova — 20.9.2026, u kodu, ostaje provera uživo

Vlasnikova stavka 2 iz `zadaci.md`. Dve operacije, **nijedna izmena na
serveru**:

* **„Add parts from a tutorial…"** — izaberi tutorijal, čekiraj njegove delove,
  oni dođu u onaj koji pišeš. Nacrt koji drži samo prazan deo se **zameni**
  umesto da se doda iza njega.
* **„Take parts into a new tutorial…"** — čekiraj delove ovog tutorijala, daj
  ime, i nov tutorijal se **odmah sačuva**; poruka nudi „Open". Ekran ne skače.

Spajanje A + B u treći je nov tutorijal pa prva operacija dvaput — nije posebna
funkcija.

**Odluke vlasnika (20.9.2026).** Kopira se, ne premešta: premeštanje je dva
upisa i drugi može da padne posle prvog, a ko hoće da ih nema u izvoru briše ih
dugmetom koje već postoji. I: izdvojen tutorijal se čuva odmah, umesto da se
otvori nesačuvan — studio drži jedan nacrt, pa bi otvaranje moralo da pita šta
sa nesačuvanim izmenama u onom koji se piše.

**Gde su vrata, i zašto baš tu.** Mala ikona desno od naslova „Tutorial
contents" (20 × 20), a na telefonu u meniju „More". Tri površine su izmerene i
sve tri su pune: Wrap tog panela ima 2 px na 840 dp, red sa naslovom 19 px
premalo, gornja traka 33 px premalo. Traka je mogla da se plati time što
„Preview tutorial" postaje ikona ispod 960 — ali to je vlasnikovo pravilo od
11.9.2026 koje drži `tutorial_editor_door_test` na 840, pa je traka vraćena
netaknuta. **Ako je 20 × 20 premalo za rad, to je prijava i znači da panel treba
preurediti** — ne da vrata treba pomeriti preko nečega što se već hvata u jedan
klik.

Mereno: aplikacija **3353 → 3383**, 1 preskočen, pun prolaz sam; `flutter
analyze` istih 26 `info`. Jedanaest mutacija, svaka crvena na svom testu.
Detalji u `docs/LESSONS.md`. Provera uživo: **stavka 202**.
...
2. **„Make exercise" → „Find the move" traži da je rešenje već odigrano na tabli,
   ...
   **Odlučeno 20.9.2026 — `PLAN-EXERCISE.md`, §10, faze 13–16. Sve četiri su u kodu:
   faze 13, 14 i 15 (aplikacija 3283 → 3295 → 3307 → 3334, server 1612/1524 → 1614/1526 →
   1632/1539; provera uživo: stavke 195, 196 i 197 — vlasnik ih je video 20.9.2026, QA
   dnevnik, osim tačke 195.6). **Faza 16 je u kodu 20.9.2026**: jedini stari
   zadatak sa linijom (`ex_69822c23d741397c`) obrisan je uz vlasnikovo „da",
   mašinerija za više poteza je obrisana na oba kraja, žica pokušaja je jedan
   potez (aplikacija 3334 → 3327, server 1632/1539 → 1624/1530; provera uživo:
   stavka 198 — i server i aplikacija moraju da budu novi). Ovo pitanje je time zatvoreno
...
**Problem.** Ručno napravljen „Find the move" zadatak (`ex_54d406e054660c5b`,
rešenje `Qf7#` ili `Qa7` — ispravno), poslat direktno i u domaćem, otvarao se i
učeniku i treneru na „Assignment complete. Your trainer can see the result." —
a niko ga nikad nije pokušao. Dva kraja jedne greške:

- **Server**: `getAssignmentDetail` je trenerovu poziciju prepoznavao po
  `id.startsWith('cust_')` — pravilo iz vremena kad je sken bio jedini pisac
  `custom_puzzles`. Ručni zadatak je `ex_…`, zadatak iz grešaka `hw_…`; oba su
  putovala bez svoje table. Nijedan test nije gledao `customPositions`. Sada
  `loadCustomPositions` **pita tabelu, ne čita id** (prefiks nije kolona — reči
  samog plana, §3); rešenje i dalje ne putuje. Provereno na pravom zadatku 71,
  samo čitanjem: stiže tabla, bez rešenja.
- **Aplikacija**: ekran taktike je taj id shvatio kao Lichess zadatak, nije mogao
  da ga učita, **preskočio ga ćutke** i na kraju liste objavio uspeh. Preskakanje
  jednog lošeg reda ostaje; kraj na kom je išta bilo neučitljivo sada kaže
  „N puzzles could not be loaded. Nothing was sent for them… Tell your trainer."

Zadaci 70, 71, 73 i 75 treba da se otvore na tabli posle restarta servera, bez
ponovnog slanja.

**Sugestija.** „Play it out" sada kaže ko je na potezu: „Your move" / „The engine
is thinking…", rečima i oblikom ikone (ruka / peščani sat), i nestaje kad je
partija gotova. Čita se sa same table (strana na potezu naspram strane koju
učenik igra), pa važi od prvog kadra. Uspravno stoji ispod cilja; položeno
**ispred** cilja u jednom redu, da tri tačke nikad ne pojedu baš to.

Mereno: aplikacija **3283** (3275 + 5 + 3), 1 preskočen, `flutter analyze` na 26
poznatih info; backend **1607** sa bazom / **1520** bez (1603/1516 + 4). Mutacije
na sve tri izmene, svaka crvena na pravom testu; jedna preživela je pokazala dva
...
nalaza je u `PLAN-EXERCISE.md` §9, faze koje iz njih slede u tabeli §7 (8–12).
Ukratko: **mašinerija radi, a nijednom čitaocu se ne kaže dovoljno.**

**Odluka vlasnika (19.9.2026), menja odluku 6 plana:** broj poteza uz **Win**
znači **„Checkmate in N moves"** — mat na tabli u N učenikovih poteza, inače
cilj nije ispunjen. „Zadrži dobijenu poziciju N poteza" više ne postoji. Razlog
je verifikacija: mat sude sama pravila, bez tablebase-a i bez granice od sedam
figura, pa trener može da zada središnjicu „matiraj u 25". Uz **Draw or
better** broj i dalje znači „for N moves" i sudi ga tablebase.

**Faza 8 — u kodu (vođa, 19.9.2026):**

- Pravilo na oba kraja i u zajedničkom fixture-u
  (`docs/gates/engine_game_cases.json`, `forMoves`): `judgeEngineGame` za win
  nikad ne traži tablebase, `goalMetByTablebase` **baca** ako ga neko pita za
  win, `parseEngineGameTask` više ne odbija win sa brojem na punoj tabli; u
  aplikaciji isto (`engineGameVerdict`, `ExerciseJudge.mateInMoves` umesto
  `refused`). Tri nova slučaja: promašen mat u dva sa tri figure na tabli (gde
  bi tablebase rekao „dobijeno"), mat dat baš u N-tom potezu, i mat u N na 32
  figure.
- **Zadatak se kaže svuda gde se čita**: naslov stavke domaćeg („Play it out:
  checkmate in 5 moves" — `childTitle`), traka iznad table sa preostalim
  potezima („You are White — checkmate in 5 moves · 3 left" —
  `engineGameGoalSentence`), i završni dijalog („The game ended: no checkmate
  in 5 moves." — `engineGameEndingWords`). List za pravljenje zadatka pod
  „Win" piše „Checkmate in N moves", pod „Draw or better" ostaje „For N moves".
  Naslovi **već poslatih** domaćih ostaju stari — naslov se piše pri slanju.
- **„0 of 0 items"**: detalj domaćeg sada šalje `child_total` i
  `child_completed`, brojano nad istim redovima koje već čita.

Mereno: aplikacija **3195** (3190 + 5: dva testa reči na tabli, jedan test ekrana
za promašen mat, dva nova slučaja iz fixture-a), 1 preskočen, `flutter analyze`
nepromenjen na 26 poznatih info; backend **1597** sa bazom / **1511** bez (bilo 1598 / 1512: fajl
`engine_game_for_moves` 22 → 21 — dva nova slučaja, pet redova tablebase-a za
win manje, nov test da funkcija baca i nov test naslova stavke; 1511 je mereno
bez `.env`, 1597 je 1596 mereno sa bazom + taj jedan čist test). Osam mutacija, svaka crvena na
pravom testu. **Pola sa pravom bazom je uhvatilo ono što pola bez nje nije
moglo**: četiri testa u `homework_gate` su stajala na „win kept for N moves" i
pala tek kad je podignut privremeni klaster — prebačena su na „draw held", a
promašeni mat je dodat u test „pravila sude, niko se ne pita".

**Faza 9, serverska polovina — u kodu (vođa, 19.9.2026, `7065ab9`).** Mereno
pre pisanja: potezi, završetak, sudija i presuda **već se čuvaju**
(`assignment_items.game_moves / game_ending / judged_by / solved`), a pregled
nije čitao ništa od toga — stavka partije nema `puzzle_id`, pa ju je `shapeItem`
oblikovao kao korak lekcije bez koraka; otud „board not available", „viewed" i
„correct 0". Bez izmene šeme. Pregled sada šalje `kind: 'game'`: zadatu poziciju,
zadatak u obliku koji čita `exerciseTaskWords`, poteze, dostignutu poziciju
(odigrano na serveru; `null` ako se potezi ne mogu odigrati), završetak,
`judgedBy`, presudu i `pending`. Red domaćeg ne traži ništa od servera — dete već
nosi svoje brojače, samo ih red nikad nije izgovorio. Backend **1603** sa bazom /
**1516** bez (1597/1511 + 5 čistih testova + 1 na pravoj bazi). **Polovina u
aplikaciji — u kodu (19.9.2026)**: gradio `implementer` u worktree-u prema
kapiji vođe `chess_app/test/homework_game_review_test.dart`; ocenjeno mašinom
(kapija bajt-identična, tri fajla i ništa više, bez novog `ignore`, pet
mutacija — četiri crvene na pravom testu, jedna **preživela**: ništa nije
proveravalo da su table okrenute na stranu koju je učenik igrao; rupa je bila u
kapiji, ne u radu, i kapija to sada traži). Pregled partije je kartica: zadatak
rečima, presuda rečima i oblikom ikone, potezi numerisani od pozicije, „Start"
i „Position reached", „Ended: …", „Judged by …", i dve poštene rečenice — „još
nije presuđeno" i „proveravano je samo da nije matiran" — obe se završavaju sa
„the position reached is yours to judge". Red domaćeg kaže „Goal met" / „Goal
not met" / „N of M correct" i treneru i učeniku; trenerov pritisak na odigranu
stavku otvara pregled, ne tablu. Aplikacija **3212** (3195 + 17 testova kapije), 1 preskočen, `flutter analyze`
...
**Faza 11 — u kodu (19.9.2026), samo aplikacija, bez restarta servera.** Mereno
pre pisanja: server ima sve još od faze 2a (`GET` i `PUT /exercises/:id`, testirano
i na ručno napravljenom i na skeniranom redu), aplikacija ima `load` i `update`
koje niko nije zvao. Odluke vlasnika: **svoj ekran sa tablom**, i **svaki moj
zadatak** (ručni, i sken sa odštampanim rešenjem). Pritisak na svoj zadatak u
Library otvara `ExerciseEditorScreen`: ime, zadatak rečima, tabla okrenuta na
učenikovu stranu, rešenje kao linija; potez odigran na tabli na izabranom koraku
postaje prihvaćena alternativa (`ExerciseLineEdit`, koji nema svoje pravilo — svaku
izmenu pita `ExerciseLine.read`); alternativa se uklanja, glavni potez ne. „Save"
otvara postojeći list u režimu izmene (`MakeExerciseSheet.edit`): popunjen, **vrsta
zaključana** (find poslat u domaćem sudi se iz ovog reda, potez po potez; zadatak
partije se kopira u domaći pri slanju, pa Win ↔ Draw or better sme), `PUT` **bez
`fen`**, `thinkSeconds` putuje netaknut. Zadatak trenera i goli sken otvaraju se
kao do sada. List za pravljenje sada kaže da je varijanta na učenikovom potezu
prihvaćena alternativa (`kVariationHint`). Gradio `implementer` prema kapiji
`docs/gates/exercise_edit_test.dart`; ocenjeno mašinom. **Ocenjivanje je našlo
jednu grešku, a mutacije dve rupe — sve tri u kapiji vođe**: tabla partije se
okretala po strani na potezu umesto po strani koju učenik igra; „glavni potez se ne
uklanja" je stajalo na liniji gde ga je odbijao *čitalac*, pa se čuvar mogao
obrisati neprimećeno; i niko nije gledao tablu koja se vidi posle izbora koraka.
`test/exercise_edit_lead_test.dart` (4) sada pita sve tri; 17 mutacija, svaka crvena
na pravom testu. Aplikacija **3257** (3218 + 24 kapija + 11 radnikovih + 4 vođe),
1 preskočen, `flutter analyze` nepromenjen na 26 poznatih info. Provera uživo:
**stavka 192**.

**Faza 12 — u kodu (vođa, 19.9.2026), samo aplikacija, bez restarta servera.**
Svaka stavka domaćeg ide kroz jedna vrata (`assignmentItemScreen`) na četiri
ekrana. `ChessBoardWithOverlay.copyPosition` (podrazumevano uključeno) gasi desni
klik i Ctrl+C; **zatvorena tabla i dalje zauzima vrh `BoardOnScreen` i tamo kaže
...
**Faza 6 zatvorena bez gradnje (odluka vlasnika, 19.9.2026):** presuda sa
učenikovog motora nam ne treba — gde tablebase ne odgovara, **trener preuzima
ulogu motora i donosi sud**, a faza 9 mu za to daje poteze i dostignutu poziciju.
Plan `PLAN-EXERCISE.md` je time ceo u kodu.

**Otvoreno:** samo provera uživo.

Provera uživo: **stavke 189–193**.
...
`docs/PLAN-EXERCISE.md`; šest odluka je u njegovom §2. Ukratko: reč je
**Exercise** (Task ostaje polje — šta se traži); čip „from book" postaje
„Exercises" (šest čipova ostaje šest, poreklo i cilj su filteri); gola pozicija
ne može u domaći; liste crtaju tablu postojećim `BoardThumbnail`; pogrešan
potez u nizu je „probaj ponovo", a izveštaj pamti prvi pokušaj; motor i
tablebase proveravaju zadatak **kad se pravi**, ne kad se rešava; partija „na
N poteza" sudi se po poziciji u koju se stiglo — server već ima i poteze i
`tablebaseService`, pa je to jedan upit na kraju, sa trećim odgovorom
„odigrano, još nije presuđeno" kad tablebase ćuti.

Izmereno pre pisanja: `custom_puzzles` ima samo dva pisca (skener i domaći iz
grešaka), pa trener **ne može rukom da napravi** poziciju kojoj se sudi —
„Save position" iz Preparation piše u `saved_lessons`. To je rupa, ne sudije.

**Faza 1 u kodu** (18.9.2026, vođa; `bd5bf05`). `custom_puzzles` je
dobio `name`, `origin` (NOT NULL **bez** podrazumevane vrednosti, CHECK),
`task` i `solution`; `services/exercise.js` je jedino mesto koje zna šta red
traži (`exerciseOf`, `exerciseColumns`, `readSolution`, `firstMoveOf`,
`assignableProblem` preseljen iz `customPuzzleJudge`). Šest potrošača čita
kroz njega; skener i domaći iz grešaka ostaju **pisci** `solution_san` i
upisuju `origin`. Stari redovi se ne diraju: NULL `task` i `solution` znače
„nađi potez, a potez je `solution_san`". Migracija je prošla na upravljanoj
bazi — rađeno je u worktree-u jer vlasnikov nodemon pokreće `initDB` na svako
snimanje, pa je sve stiglo kao jedan patch; novi proces sluša posle
`await initDB()`. Backend 1503 → 1522 sa bazom, 1441 → 1454 bez; 14 mutacija,
svaka crvena na pravom testu. Tri su prvo **preživele**: nijedan test nije
puštao pisce na pravu tabelu, pa bi pisac koji zaboravi `origin` bio zelen u
testovima, a 500 na svaki potvrđen sken. Aplikacija nije dirana ni puštana.

Vlasnik, istog dana: aplikacija je u testiranju, on je jedini korisnik, **stari
domaći mogu da se brišu i ne treba im se prilagođavati** (odluka 7 u planu).
Ne važi za njegove skenirane pozicije.

**Faza 2a u kodu** (18.9.2026, vođa). `judgeLine` sudi **svaki** potez sa
spiska, ne samo poslednji, i nastavlja od autorovog poteza i kad je odigrana
prihvaćena alternativa (`continuesOn`). Ruta `custom-attempt` prima `moves`,
vraća po jedan odgovor protivnika i upisuje presudu **jednom** — na prvom
pogrešnom potezu ili na kraju niza; niz nikad ne otkriva rešenje.
`services/exerciseAuthoring.js` + `routes/exercises.js` (POST, GET jedan, PUT):
čuva se ono što čitaoci pročitaju nazad, pozicija zadatka se ne menja (409),
tuđe je 404. Zajednički fixture `docs/gates/exercise_line_cases.json` — na
njemu će stajati i aplikacija u 2b; žica je opisana u §7a plana. Backend
1522 → 1565 sa bazom, 1454 → 1486 bez; 17 mutacija. Dve su prvo preživele:
...
**Faza 3a u kodu** (18.9.2026, vođa). Partija „na N poteza" sada važi i za
`win` i za `hold`, a kad stane na N-tom potezu sudi joj se po poziciji u koju je
stigla: sa sedam figura ili manje server jednom pita tablebase
(`recordEngineGameResult` → `askTablebase`), iz ugla učenika, a „cursed win" i
„blessed loss" su remi. Tri odgovora: ispunjeno, nije ispunjeno, **bez odgovora**
— tada je partija odigrana i važi za kapiju, `solved` i `judged_by` su NULL, i
sledeće čitanje domaćeg (bilo koje strane) pita ponovo; to pitanje ne može da
obori čitanje. Samo `TablebaseUnavailable` je „bez odgovora" — greška u kodu
puca glasno i ništa ne upisuje. `win` na N poteza sa više od sedam figura se
**odbija** pri pisanju. Polje **nije** preimenovano u `forMoves`: aplikacija na
master-u čita i piše `surviveMoves`, pa bi preimenovanje samo na serveru
razdvojilo dva kraja do faze 3b. `childrenOf` šalje `pending_items` za 3b.
Backend 1564 → 1592 sa bazom, 1486 → 1508 bez; 15 mutacija, nijedna nije
preživela.

**Faza 2b u kodu** (18.9.2026, implementer; ocenio i preuzeo vođa). „Make
exercise" pored „Save position" u Preparation: rešenje se čita iz stabla **od
korena**, varijante na učenikovim potezima su prihvaćene alternative, poslednji
protivnikov potez se odbacuje i to se kaže; rešavač igra niz — jedan odgovor po
...
**Faze 3b i 4 u kodu** (19.9.2026, dva implementera; ocenio i preuzeo vođa).
3b: list „Make exercise" prvo pita šta se traži (nađi potez / pobeda / remi ili
bolje), za partiju još koliko dugo i koju stranu igra učenik (ništa nije unapred
izabrano), i kaže ko će suditi; ekran vežbi na N-tom potezu čeka server i kaže
**njegovu** presudu ili „not judged yet"; red domaćeg piše „Played — not judged
yet". 4: sedmi čip „Exercises" sa dva filtera (šta traži, odakle je),
„Positions" samo za gole pozicije, sličica table u redovima i pregled na dodir
(ko je na potezu — rečima), u domaćem „Add" nudi tri stvari umesto četiri i
gola pozicija više ne može u domaći; „Play it out" dijalog sa lepljenjem FEN-a
je obrisan. Aplikacija 3083 → 3125, analyze istih 26, backend 1594 sa bazom.
...
lažni. Provera uživo: stavka 187. Faza 6 je zatvorena bez gradnje 19.9.2026 (sudi trener).
