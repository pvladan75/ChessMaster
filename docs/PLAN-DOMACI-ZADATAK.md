...
| 3b | **Authoring, the app half**: model, API service, the editor with reorder and the two switches, and the doors — a Teach card and a Library chip | implementer — brief: `docs/briefs/BRIEF-DOMACI-FAZA3-APP.md` | `docs/gates/homework_editor_test.dart`, **red on master today**, copied into `test/` and left green; plus reachability pumped (not counted in the source), one item of each kind, a refusal shown, and no gate switch on the first row |
