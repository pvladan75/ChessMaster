...
| 2b | **Play it out, the app half**: the task and its verdict in the app, the exercise screen playing an assigned game at *the task's* strength with the opponent sheet absent, ending by the verdict, posting the moves | implementer — brief: `docs/briefs/BRIEF-DOMACI-FAZA2-APP.md` | `docs/gates/engine_game_goal_test.dart`, **red on master today** (the model does not exist), copied into `test/` and left green; plus a widget test per goal, the request asserted to carry no verdict, and the opponent button absent on an assigned game |
