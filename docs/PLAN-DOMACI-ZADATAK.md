...
| 1 | **Schema**: `homeworks`, `homework_items`, `assignments.parent_id/position/gate/gate_opened_at`, `kind += 'homework', 'engine_game'`; parent completion in `markCompleteIfDone`; `GET /assignments/:id` of a parent returns its children with state | lead (schema is irreversible) | backend tests: a child finishing completes the parent only when all do; a gated child is reported locked until the previous is done; `.env` moved aside |
| 2 | **Play it out**: `engine_game` assignment carrying fen, side, goal, level, think time, ply cap; the exercise screen opens it, plays at *that* strength with the opponent sheet absent, ends by the verdict, judges the goal, records result and moves | implementer | a widget test per goal — win met, win missed, hold met by draw, survive met at the cap, survive missed by mate; the opponent sheet is not found on an assigned game (`engine_opponent_sheet_test` widened) |
| 3 | **Authoring**: Library kind, Teach card, editor with reorder and gate switches, `POST/PUT /homeworks` | implementer | items keep their `item_key` across a reorder (mutation: renumber and the test fails); 360 dp portrait and landscape fit |
| 4 | **Send**: one dialog → parent + children in one transaction, quota per §8; sent copies listed under the homework | lead (quota) | a failed child creates nothing (transaction proven by a forced failure); quota refunded on failure |
| 5 | **The student's homework screen** and its row; the gate drawn, the escape hatch from the review screen | implementer | locked item not openable and named after its blocker; opened by the trainer, it opens for that student only |
