...
| 14 | **Built 20.9.2026 by the lead, inline**, with the owner's server stopped. Server: the cap is the **writer's** rule — `parseExercise` refuses a find solution of more than one step, *after* `readSolution` has read it, so a line that does not replay is told so rather than told it is long; `readSolution`, `exerciseOf` and the judge are untouched and rows that exist are read as the lines they are. The fixture gained `solutions.scholarFirst` and `oneMove` (the refusal, which solutions are refused for length alone, which are taken). **Differs from the row as written in one thing**: the app's reader `ExerciseLine.read` keeps its limit of 20 — it reads saved lines too, until phase 16 — and the app cannot *write* a longer one by construction: `fromTree` reads the root's children and nothing under them (`laterMovesIgnored` replaces `droppedReply` / `ignoredReplies`, and the sheet says „Only the first move is asked"), and the making screen holds one step. App: `ExerciseLineEdit.empty` / `play` / `clear`; `ExerciseEditorScreen.make(fen:)` — first move the answer, further ones alternatives, „Start over", Save off until a move is played, the unsaved-change question as in an edit; `MakeExerciseSheet.withAnswer` saves it with the position; the sheet over an empty tree shows **„Play the move"** under Find alone, and closes with what that screen saved so the room hears of one exercise, once. `test/exercise_one_move_test.dart` (17); the 2b gate's writer group (5) moved there re-aimed; two older tests re-aimed, one of them an assertion of absence of a sentence that no longer exists anywhere. 4 + 11 mutations, each red on the right test. Backend 1612/1524 → 1614/1526, app 3295 → 3307, analyze the same 26 infos. Live: item 196. *As planned:* **Find is one move, and it is made on the exercise's screen.** Server: `POST`/`PUT /exercises` refuse a find solution of more than one step, in words („A find exercise asks for one move. For more, use Checkmate in N or Play N moves."); the judge of rows that exist is not touched here. App: `ExerciseLine`'s limit follows, on the shared fixture `exercise_line_cases.json`; `fromTree` reads **the root's children only** — the first is the answer, its siblings the alternatives — and says when deeper moves were not used. `ExerciseEditorScreen` gains a making mode opened on a position with no answer yet: the first move played on its board is the answer, every further one an alternative, the answer replaceable until saved; Save opens the sheet that exists. In the sheet, *Find* with no move on the tree shows **„Play the move" → that screen** instead of the red refusal; a move already on the tree is carried in. The sheet's chip reads „Find the move" | lead (server, fixture), then `[implementer]` | fixture: a two-step line refused on both ends with the same words, a one-step line with alternatives accepted; widget test **from an empty tree**: the door, the button, two moves played through the board's own `onMove`, and the `POST` body asserted — `solution` is one step with both moves; the old sentence „Play the solution on the board first" grepped out of `lib/` and the tests; 360 × 640. Live: a new item in `TODO-provera.md` |
| 15 | **Built 20.9.2026 by the lead, inline**, with the owner's server stopped. Server: `play` is a fourth goal that needs its number (`NEEDS_MOVES`); `judgeEngineGame` answers `goalMet: null` and `needsTrainer: true` for it **however the game ended** — mated, mating, resigned, stopped at its number — and never `needsTablebase`; `goalMetByTablebase` throws if asked about one, as it does for a win; `recordEngineGameResult` stores it played with no verdict. `recordTrainerVerdict` + `POST /assignments/:id/game-verdict` (`{met: true|false}`, anything else 400): one `UPDATE` that writes only where the assignment is the trainer's own **and** `trainerOwnsStudent` holds **and** the game was played **and** nobody but the trainer has judged it — 404 for the first three, 409 for the last; the trainer's own word may be changed. It serves every game left pending, so phase 9's „yours to judge" has its button. `judged_by` gains `trainer` (the constraint is dropped and re-added by `initDB` on every start, so it is an edit of one list and touches no row). **Found on the way, not in the row as written**: „pending" was spelled `game_ending = 'moveTarget'` in two places — `childrenOf`'s `pending_items` and the review's `pending` — so a game with no goal that ended in mate would have been neither judged nor waiting; both now ask for *an* ending. The row's „1–50" is the sheet's field; the server keeps its one limit of 200 for every goal. App: `EngineGameGoal.play`, `EngineGameVerdict.needsTrainer`, `engineGameSaid` answers „not judged yet" for it whatever the server said; `ExerciseAsk.play` (so the Library's task filter has it too), `ExerciseJudge.trainer`, the words in their one home; the sheet's fourth chip asks the number only and checks nothing; the closing dialog adds „Your trainer will look at it."; the review card says who will judge, to each reader in their own words, and gives the trainer **„Mark as met" / „Mark as not met"** — worded as the act, because phase 9's gate rightly holds that an unjudged card never reads „Goal met" — then reads the review again, so the card shows what was stored. `engine_game_play.test.js` (12), five more in `homework_gate.test.js`, one in `assignment_review.test.js`, `test/exercise_play_moves_test.dart` (27); 11 + 19 mutations — one survived, a condition (`item.attempted`) that the server's `pending` already implies, and it was removed rather than tested. Backend 1614/1526 → 1632/1539, app 3307 → 3334, analyze the same 26 infos. Live: item 197. *As planned:* **Play N moves — the trainer judges.** A game task with no goal: `goal: 'play'`, `surviveMoves` required (1–50), side and engine strength as for the others. The game stops at N of the student's moves. It is **never sent to the tablebase** — not on the result route and not by the retry on read — and rests *played, not judged* until the trainer says. **The trainer's verdict gets a route** (`trainerOwnsStudent`, nothing else): met / not met, written as `judged_by = 'trainer'`, allowed on any game item still pending — which also gives the „yours to judge" sentence of phase 9 the button it never had. A mate or a draw inside the N moves ends the game as the rules say and is shown as it is; the verdict is still the trainer's. Schema: `judged_by` gains `trainer` (lead; check the constraint and what `initDB` does to it before the owner's nodemon restarts on the save — rule 20). App: the fourth chip in the sheet, the words in `exercise_task_words.dart` („Play N moves as White — your trainer will look at the game"), the two verdict buttons on the review card, the row's „not judged yet" until then | lead (schema, route, judge), then `[implementer]` | shared fixture `engine_game_cases.json` extended: `play` parses with N and is refused without; at the move target it is pending with no tablebase request — **asserted on the fake client's request log**, with few pieces on, where a tablebase *would* answer; the retry on read leaves it alone (real database); the verdict route: the trainer of an accepted edge writes it, a pending-edge trainer and a stranger get 404, a second verdict overwrites and says who; mutations on each. App: gate file over the same fixture; the verdict buttons absent for the student |
| 16 | **Built 20.9.2026 by the lead, inline**, with the owner's server stopped, after the owner said 195–197 were watched (his QA log has them „ok“, one line of 195 unanswered). **The count first**: one hand-made row held more than one step — `ex_69822c23d741397c`, K+Q v K, six steps, in no homework — shown to the owner and deleted on his yes, in a transaction that committed only on exactly that row; 112 rows left, none with a line. Server: `judgeLine` is gone and the attempt route judges through `firstMoveOf` + `judgeAttempt`; its body is `{puzzleId, moveSan, msTaken}` and its answer `{correct, reason, playedSan, solutionSan}` — `moves`, `done`, `step`, `reply`, `continuesOn`, `retry` left the wire, so an old app on a new server gets a 400, not a wrong verdict. `readSolution` reads **a list of exactly one** `{accept}`: the cap moved from the writer into the reader (one rule, one home), and a row that still held a line would be refused aloud and never judged on its first move. **The stored shape stays a list**, so no row is rewritten; a stored `reply: null` is simply not read. App: `ExerciseLinePlay` deleted, `ExerciseStep` is `accept` alone, `ExerciseLine.read` says the server's sentence, `ExerciseLineEdit` is `play` / `remove` / `clear`, the editor lost its step chips and `fenBefore`, the solver sends one move and locks, the check on save asks the tablebase about one position (`studentFens` and `ExerciseFinding.step` gone), „Find the moves“ is no longer a sentence. The fixture is one-move throughout: 11 judged, 6 refused. 4 mutations on the server (two on the reader, two on the route, the latter on the real database), 6 on the app — two survived their first run and were questions about the tests (a board reset asserted on a board that never moved; a lock kept twice), both answered. Backend 1632/1539 → 1624/1530, app 3334 → 3327, analyze the same 26 infos. Live: item 198. **Not touched**: `docs/gates/*.dart`, the gates as they were handed over — history, outside this row's grep. *As planned:* **Delete what the cap made dead** — after 13–15 are watched running, as one batch the owner asked to be kept separate: the multi-step half of `judgeLine` (`reply`, `continuesOn`), the solver's line playing, the step chips of `ExerciseEditorScreen`, the wire fields of §7a that carry them. Before it: count hand-made rows with more than one step, show the owner the list, delete them on a yes | lead | both suites at their new floors with the arithmetic in `LESSONS.md`; `git grep` for each removed name comes back empty in `lib/`, `services/`, `routes/` and the tests |
