...
//
// Written when `review_items` and `assignment_items` keyed a student's
// schedule and recorded answers to a part's *index*, so inserting a part at
// the front silently re-aimed every row. Both keys went with phase 2 of
// docs/PLAN-TUTORIJAL-VIDEO.md; what these cases still hold is the rule itself
// — a part keeps its id across edits and saves, which the app relies on
// (`PUT /lessons/:id` refuses a save that would drop the stored ids) and any
// future reader by id will.
...
test('an id that arrives is kept, never regenerated', () => {
  ...
  // has to come out the other side as the same step, or anything naming it
  // by id is orphaned — and orphaned silently, since nothing joins on it.
...
test('an id that could not have been written here is refused', () => {
  ...
  // is exactly how an identity would be lost without anybody noticing.
...
test('a duplicate id inside one lesson is refused', () => {
  // Two steps claiming one id is two parts with one identity; anything reading
  // by id would find whichever came first. Refused at the door.
  const built = buildLessonSteps([
    { id: 'aaaa1111', fen: FEN },
...
test('a stored step with no id is backfilled from its index', () => {
  // `p<index>` and not a random id, because a part's id must be the same on
  // every read of the same lesson. A generator here would hand out a different
  // one each time.
...
test('inserting a step at the front does not rename the steps after it', () => {
  // The regression this whole phase existed for. Before: the second step was
  // `position 1`. After the insert it is `position 2`, and anything that named
  // it by index now points at another board.
...
test('a key whose step is gone resolves to null, never to a neighbour', () => {
  // A part that was deleted is not whichever part inherited its index:
  // answering with a neighbour would be worse than answering nothing.
  const steps = stepsOfLesson({
