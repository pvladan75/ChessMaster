...
// The local Syzygy sidecar. It exists because the endgame audit is a scan of a
// donated service: 471 games of one real archive, and three runs died on a 429.
// What matters in these four is that it never becomes a silent detour — a
// sidecar that is configured and not answering must still produce right
// answers, slowly, and say so.

/// Answers `local` for anything, or 404, or refuses the connection.
function stubSidecar({ mode = 'answers' } = {}) {
  let sidecarCalls = 0;
  let lichessCalls = 0;
  const fetchImpl = async (url) => {
    if (url.startsWith('http://side')) {
      sidecarCalls += 1;
      if (mode === 'down') throw new Error('ECONNREFUSED');
      if (mode === 'declines') return { ok: false, status: 404, json: async () => ({ covered: false }) };
      return { ok: true, status: 200, json: async () => PAWN_ENDING_REPLY };
    }
    lichessCalls += 1;
    return { ok: true, status: 200, json: async () => PAWN_ENDING_REPLY };
  };
  return {
    fetchImpl,
    sidecarCalls: () => sidecarCalls,
    lichessCalls: () => lichessCalls,
  };
}

test('a position the local tables can answer never reaches Lichess', async () => {
  const stub = stubSidecar();
  const tb = createTablebase({
    fetchImpl: stub.fetchImpl, sidecarUrl: 'http://side/standard', ...fakeClock(),
  });

  const result = await tb.probe(PAWN_ENDING);

  assert.equal(result.category, 'win');
  assert.equal(stub.sidecarCalls(), 1);
  assert.equal(stub.lichessCalls(), 0, 'the whole point is not asking');
  assert.equal(tb.stats().local, 1);
});

test('a sidecar that declines hands the position to Lichess', async () => {
  // 404 is the sidecar refusing — castling, past its largest table, a table
  // missing from disk, or a verdict the fifty-move rule could overturn. Those
  // are exactly the positions it must not answer, and they still need one.
  const stub = stubSidecar({ mode: 'declines' });
  const tb = createTablebase({
    fetchImpl: stub.fetchImpl, sidecarUrl: 'http://side/standard', ...fakeClock(),
  });
  ...
  assert.equal(result.category, 'win');
  assert.equal(stub.lichessCalls(), 1);
  assert.equal(tb.stats().declined, 1);
  assert.equal(tb.stats().local, 0);
});

test('a sidecar that is down is a slower answer, never a wrong one', async () => {
  const stub = stubSidecar({ mode: 'down' });
  const tb = createTablebase({
    fetchImpl: stub.fetchImpl, sidecarUrl: 'http://side/standard', ...fakeClock(),
  });
  ...
  assert.equal(result.category, 'win', 'the position is still judged');
  assert.equal(stub.lichessCalls(), 1);
  assert.equal(
    tb.stats().unreachable, 1,
    'counted, because "configured and dead" must not read like "not configured"',
  );
});

test('local answers are not spaced, because there is nothing to be polite to', async () => {
  const stub = stubSidecar();
  const clock = fakeClock();
  const tb = createTablebase({
    fetchImpl: stub.fetchImpl,
    sidecarUrl: 'http://side/standard',
    minGapMs: 1000,
    now: clock.now,
    sleep: clock.sleep,
  });
  ...
  assert.equal(stub.sidecarCalls(), 2);
  assert.equal(
    clock.read() - started, 0,
    'pacing a memory-mapped file on the same machine would pace a disk read',
  );
});
