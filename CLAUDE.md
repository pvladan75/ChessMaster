...
```bash
cd chess_app && flutter test          # 971 tests, 1 skipped, rest green
cd chess_app && flutter analyze       # exits 1 on 29 known infos — read the list
cd chess_backend && npm test          # node --test, 738 tests, all green
cd chess_backend && npm run dev       # nodemon, port 3000
```

Both counts measured 31.8.2026, after the repertoire line-drill work. They are here so a suite that quietly stops running half of itself is
