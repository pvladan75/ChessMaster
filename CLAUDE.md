...
```bash
cd chess_app && flutter test          # 2718 tests, 1 skipped, rest green
cd chess_app && flutter analyze       # exits 1 on 26 known infos — read the list
cd chess_backend && npm test          # node --test, 1376 tests, all green
cd chess_backend && npm run dev       # nodemon, port 3000
```

Measured on `master` on 16.9.2026, after block C of the architecture audit, the
owner's four decisions on it, and the merge of `feat/repertoire-engine-count-pgn`, and the Analysis PGN import
(`docs/STANJE-RADA.md`, „Analiza uvozi PGN sa varijantama"). Every change of these numbers,
