...
cd chess_backend && npm test          # node --test, 1522 with TEST_DATABASE_URL, 1454 without
...
and phases 1–2 of the puzzle progress plan; then phase 1 of `docs/PLAN-EXERCISE.md`, which touched the backend only (the app's count is from before it and unchanged). Only the owner's live pass is open. Every change of these numbers,
