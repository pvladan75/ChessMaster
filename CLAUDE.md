...
```bash
cd chess_app && flutter test          # 4103 tests, 1 skipped, rest green
...
Then the side nobody set (→ **3876**): a position stored as White with
`needs_review` is asked about before any door uses it — the Library's open,
„Make exercise" and „Add to tutorial", Saved Positions' „Add to tutorial" and
the room's board all go through `settledFen` (`side_to_move_gate.dart`). „Make
...
fixtures'**.
Then its phase 3, the words for a review (→ **4171**, a full run; backend →
**1723** measured without a database, 1874 with derived): `POST /review-words`,
the clock of a move, and „Comment key moments with AI" — every sentence through
...
and the ten model answers were recorded again. Then `best_stands_out` /
`margin_pawns`, written by both ends and read by neither skeleton, were deleted
(→ **4186**, two harness cases of the half-pawn margin gone with it), and
...
step the code undoes cannot see the undo go missing.** Then every other way
...
Then the owner read his own tutorial's map as a chain 2 → 3 → 4 and could not
tell which way its dashed lines went (→ **4103**, a full run; analyze the same
22): a return now hangs from **the move its row names**, and every line ends in
an arrowhead at the part it leads to — phase 0 of `docs/PLAN-REDOSLED-GRANA.md`,
whose phases 1–2 bring „Move variation earlier / later" to every move tree and
to a tutorial's branches. **A picture answered what a test could not**: both
