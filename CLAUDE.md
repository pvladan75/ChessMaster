...
```bash
cd chess_app && flutter test          # 2689 tests, 1 skipped, rest green
cd chess_app && flutter analyze       # exits 1 on 26 known infos — read the list
cd chess_backend && npm test          # node --test, 1376 tests, all green
cd chess_backend && npm run dev       # nodemon, port 3000
```

Measured on `master` on 16.9.2026, after block C of the architecture audit and the
owner's four decisions on it (`docs/STANJE-RADA.md`, „Ostatak revizije (blok C)"). Every change of these numbers,
